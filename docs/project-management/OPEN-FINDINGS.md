...
`TC-DASH-*`/`TC-ORACLE-001-*`/`TC-ALERT-001-*`/`TC-AI-SEARCH-001-*` on
2026-08-29 (see
...
| ID | Area | Description | Source | Status |
|---|---|---|---|---|
| F-22 | Executive Dashboard vs. Prototype P-014 (and Main Dashboard vs. P-002 — same page, same gap) | Prototype P-014 (Executive Dashboard) and P-002 (Main Dashboard) both specify the identical tile/section spec — KPI tiles named exactly "NBV", "Risk", "Utilization"/"Total Assets"/"Warranty Expiry" and sections named "Asset by Category"/"Lifecycle / Maintenance Overview"/"Recent Alerts" (`AC-EXEC-001-01`/`-02`, `AC-DASH-01..03`). Both trace to the same single built page, `frontend/src/pages/Dashboard/index.tsx` (ported from the legacy ESAPS dashboard predating this PRD/prototype), which has neither — its KPI grid is Total Assets/Available/Assigned/In Maintenance/Expired Warranty/Software Licenses/Monthly Depreciation/Monthly Cost, and its sections are AI Insights/AI Portfolio Health/Oracle FA Reconciliation/Asset Lifecycle/Department Distribution/Asset Status/Asset Type/Pending Approvals/Recent Activities/Maintenance Calendar. This is independent of the still-open NBV/Risk formula question (F-03) — even presence-only testing fails, confirmed twice from two different Prototype screens/AC groups against the exact same page. Raises a scope question worth surfacing to the business/design owner: should Prototype P-002/P-014 be updated to match the shipped legacy-derived layout, or should the Dashboard eventually grow those tiles/sections alongside what exists today? Not answered here — flagged, not resolved. | `frontend/src/pages/Dashboard/index.tsx`; `docs/03-prototype/RAISE-PROTOTYPE.md` P-002/P-014; confirmed by browser test execution 2026-08-26 (TC-EXEC-001-01/-02) and 2026-08-29 (TC-DASH-01..03) | Open |
| F-31 | Oracle FA / Financial View (P-011) not built — placeholder stub only | Prototype P-011 ("Financial View") and `AC-ORACLE-001-01..04` specify a screen with Asset Number/Acquisition Information/NBV/Depreciation/Oracle Source/Synchronization Status fields, plus 4 distinct states (available/unavailable/error/conflict). The route the app maps to `RAISE-FR-ORACLE-001` (`/reconciliation`, "Oracle FA Reconcile" in navigation) instead renders `frontend/src/pages/_shared/ModulePage.tsx` — a generic "foundation placeholder" `EmptyState` with no fields and no state logic at all ("Migrates from src/pages/Reconciliation.tsx once Oracle FA is connected in Phase 6."). The closest analog, Asset Detail's own "Financial" section (added for F-24), shows only Purchase Cost/Current Value/Purchase Date — no Oracle-specific field. This is distinct from **F-04** (the integration-mechanism question, PRD §16 Q6–Q10, still genuinely open) — even presence-only testing of the four UI states fails, independent of how the real integration would eventually work, the same pattern already established by F-22 for the Dashboard. Not a "first cut" candidate without a design/business decision on what a placeholder-vs-real Financial View should look like before Oracle is actually connected. | `frontend/src/pages/_shared/ModulePage.tsx`; `frontend/src/config/navigation.ts`; `docs/03-prototype/RAISE-PROTOTYPE.md` P-011; confirmed by browser test execution 2026-08-29 (TC-ORACLE-001-01..04) | Open |
| F-32 | Alerts (P-012) not built — the sidebar route 404s, worse than a placeholder stub | Prototype P-012 ("Alerts") and `AC-ALERT-001-01..02` specify a screen listing each alert with severity, description, and associated asset, in-app only. The sidebar nav entry "Notification Center" (`id: notifications`) routes to `/notifications`, which renders the app's generic 404 page ("Page not found — hasn't been migrated yet"), not even `ModulePage`'s placeholder pattern used for Oracle FA/License Mgmt/etc. The only related UI, the header bell-icon dropdown (`frontend/src/components/AppShell.tsx`), is hardcoded to an empty `notifications` array with a static "will be wired to the real API in a later phase" message, and its own "View all notifications" link points to the same 404 route. This is distinct from **F-05** (the trigger-rule question, PRD §6.9 Open Question, still genuinely open) — even presence-only testing of a single alert with severity/description/asset fails, since no alert list renders anywhere at all, the same pattern already established by F-22 (Dashboard) and F-31 (Oracle FA). The Dashboard's "AI Insights" → "Warranty Expiry Approaching" card is a distinct AI Decision Center feature (aggregate count, not a per-alert severity/description/asset list) and does not satisfy this AC. Not a "first cut" candidate without a design/business decision on what a placeholder-vs-real Alerts screen should look like before trigger rules are confirmed. | `frontend/src/components/AppShell.tsx`; `frontend/src/config/navigation.ts`; `docs/03-prototype/RAISE-PROTOTYPE.md` P-012; confirmed by browser test execution 2026-08-29 (TC-ALERT-001-01..02) | Open |
| F-33 | AI Assistant (P-015) doesn't answer natural-language questions — two non-matching surfaces exist instead | Prototype P-015 ("AI Assistant") and `AC-AI-SEARCH-001-01..03` specify a natural-language Q&A surface: submit a question, receive an answer with a "Sources / Data Used" section, and (for the PRD's illustrative "Which notebooks expire within 90 days?" example) a table of Asset/Warranty/Age/Maintenance/Status columns plus an affected-asset count. Two distinct "AI" surfaces exist in the built app, neither matching this: (1) the header "AI Assistant" drawer (`frontend/src/components/AppShell.tsx`) accepts no input at all — a static placeholder message only ("AI Assistant will connect to POST /api/v1/ai/chat once the AI module migration phase lands."); (2) the Assets page's "Ask AI" box (`frontend/src/pages/Assets/index.tsx`'s `handleAISearch`) is a hardcoded keyword-to-filter matcher ported from legacy ESAPS (predating this PRD/prototype) — submitting the exact PRD example question only interpreted it as `Type = Laptop` (keyword match on "notebook") and narrowed the existing Asset list; it never returns a natural-language answer, a Sources/Data Used section, or the required Asset/Warranty/Age/Maintenance/Status table. This is distinct from **F-06** (the citation-precision/format question, PRD §16 Q18, still genuinely open) — even presence-only testing of "an answer is returned" fails, the same pattern already established by F-22/F-31/F-32. Not a "first cut" candidate without a design/business decision on what a placeholder-vs-real AI Assistant should look like before real AI backend integration lands. | `frontend/src/components/AppShell.tsx`; `frontend/src/pages/Assets/index.tsx`; `docs/03-prototype/RAISE-PROTOTYPE.md` P-015; confirmed by browser test execution 2026-08-29 (TC-AI-SEARCH-001-01..03) | Open |
