...
|---|---|---|---|---|---|---|---|---|
| `RAISE-FR-ASSET-001` | Asset Registry | P0 / MVP | §4.1 Asset Management | P-003, P-004 | AC-ASSET-001, AC-ASSET-001-DETAIL | TS-ASSET-001, TS-ASSET-001-DETAIL | TC-ASSET-001-01..04, TC-ASSET-001-D-01..02 | **FAIL (partial)** — executed 2026-08-26 against the real running app: TC-ASSET-001-01 **PASS** (list displays), TC-ASSET-001-02 **PASS** (search filters), TC-ASSET-001-04 **PASS** (row click opens Detail), TC-ASSET-001-D-02 **PASS** (Detail shows only the selected asset's data, verified across two distinct assets). TC-ASSET-001-03 **FAIL** — no Category filter exists anywhere in the Assets page's Filters panel (only Status/Department/Location); the Category column header only sorts, it doesn't filter. New finding **F-23**. TC-ASSET-001-D-01 **FAIL** — Asset Detail is missing 2 of the 9 required sections entirely: no "Financial" section (purchase cost/current value aren't shown anywhere on the page, despite existing on the underlying Asset record) and no "Lifecycle" section/label. New finding **F-24**. Both new failures are independent of the pre-existing asset-master-field-list block (PRD §16 Q1). |
| `RAISE-FR-ASSET-002` | Category & Hierarchy | P0 / MVP | §4.1 Asset Management | P-005 | AC-ASSET-002 | TS-ASSET-002 | TC-ASSET-002-01..02 | **FAIL (partial)** — executed 2026-08-26: TC-ASSET-002-02 **PASS** (Category is consistent between Asset Registry and Asset Detail for the same asset, e.g. `a1` shows "IT Hardware" in both). TC-ASSET-002-01 **FAIL** — worse than the pre-existing "taxonomy TBD" block: there is no P-005 "Category & Hierarchy" screen anywhere in the app's routing/navigation at all (confirmed via `grep` across `config/navigation.ts`/`constants.ts`/`App.tsx` — zero matches), so even the *display mechanism* the original BLOCKED note assumed exists is absent. New finding **F-25**. |
| `RAISE-FR-ASSET-003` | Custody History | P0 / MVP | §4.2 Custody & Asset Operations | P-006 | AC-ASSET-003 | TS-ASSET-003 | TC-ASSET-003-01..03 | **FAIL** — executed 2026-08-26: TC-ASSET-003-01 **PASS** (current holder "Sarah Chen" displays for asset `a1`). TC-ASSET-003-02 **FAIL** — the "Assignment History" panel (Asset Detail's History tab) shows only a "current custody state" entry plus a registration entry, not a chronological list of historical transfers; no asset in the seed data has ≥2 real custody events to even attempt this against. TC-ASSET-003-03 **FAIL** — performed a real Check-in on `a1` via the UI: the prior "Assigned to Sarah Chen" entry was **replaced**, not preserved — the panel now shows only "Currently Available / Not assigned to anyone" plus the same registration entry. This directly contradicts AC-ASSET-003-02's "new entry appended; all prior entries unchanged" for the one write path (Check-in/Check-out) this AC is confirmed to cover — independent of the still-open Check-in/Check-out-exclusivity question (Gap 4) that only concerns *other* write paths. New finding **F-26**. |
