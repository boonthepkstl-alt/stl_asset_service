...
| R-09 | No Category & Hierarchy screen (P-005) | `RAISE-FR-ASSET-002`'s dedicated screen didn't exist anywhere in the app's routing or navigation — not just its taxonomy content | Added a scoped-down first cut (`frontend/src/pages/Categories/index.tsx`, route `/categories`, nav entry under "Assets"): each category expands to show the real assets registered under it — the one parent/child relationship actually confirmed anywhere in the chain. Does **not** implement Prototype P-005's illustrative sub-category tree (tracked separately as **F-27**, still open/TBD). Verified live: "IT Hardware" expands to 6 real seeded assets; clicking one navigates to its Asset Detail; sidebar nav entry reaches the screen |
