...
**Version:** 0.4 Draft (Utilization calculation mechanics resolved; RAISE-AI-DOC-001/002/003 acceptance detail resolved; RAISE-AI-DOC-004 still TBD)
...
| Description | The system shall support maintenance information as part of the asset lifecycle. |
| Business Objective | G1, G3, G4 |
| User/Actor | IT Asset |
| Priority | P0 |
| Scope | MVP |
| Acceptance Criteria | Maintenance information can be associated with an asset; maintenance history can be used as an input to asset analysis. |
| Dependencies | RAISE-FR-ASSET-001 |
| Source Reference | v0.1 draft §6.6 |
| Traceability ID | RAISE-FR-MAINT-001 |
| Open Question | Complete maintenance workflow, SLA, vendor model, and cost model are not defined — see [§16 Q14](#16-open-questions). |
...
| Open Question | API vs. file integration; synchronization frequency; data mapping; error handling; retry mechanism; integration ownership; source-of-truth rules; security mechanism — all undefined, see [§16 Q6–Q10](#16-open-questions). |
...
|---|---|
| Authentication | TBD |
| Authorization / RBAC | TBD (see [§11](#11-security--rbac)) |
| Performance | TBD |
| Availability | TBD |
| Scalability | TBD |
| Backup / Recovery | TBD |
...
**RAISE-NFR-SEC-RBAC-001 (TBD)** pending Security Design and business confirmation
(see [§16 Q21–Q23](#16-open-questions)).

*Source: v0.1 draft §13 (Security & Governance).*
...
(RAISE-FR-OPS-002) · Maintenance (RAISE-FR-MAINT-001) · Warranty
...
none are confirmed MVP requirements (see [§7](#7-ai-requirements)).
...
Product/Business decision, 2026-08-21**.*
...
14. What maintenance information is required?
...
| RAISE-FR-MAINT-001 | Maintenance | MVP | P0 | TBD (workflow detail) | v0.1 §6.6 |
...
| RAISE-NFR-SEC-RBAC-001 | Security & RBAC | TBD | TBD | TBD | v0.1 §13 |
| RAISE-FR-LIFE-001 | Asset Lifecycle Connectivity | MVP (foundation) | P0 | APPROVED | v0.1 §9 |

**Downstream sync status for newly-added IDs (2026-08-21, updated in v0.4):** This
...
- The **RBAC model** underlying "a user with appropriate permission" (RAISE-FR-OPS-002)
  and "an authorized user" (RAISE-FR-AUDIT-001, RAISE-FR-ALERT-001) — no roles are named
...
**Version:** 0.4 (Draft for Requirement Review)
**Status:** Draft for Requirement Review
...
2026-08-21, still unanswered) and confirmation items in the Pre-Finalization Quality
Pass; then run the downstream sync chain (`RAISE-DESIGN.md` onward) to reflect this v0.4
update.
