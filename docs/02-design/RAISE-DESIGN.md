...
design. **Updated 2026-09-05 (PRD v0.18, §16 Resolved Question 49):** `A7`
now also covers a second Presentation-layer surface — the header bell in
`frontend/src/components/AppShell.tsx` — which reads the same `A7`
derivation via a shared hook rather than its own node; this does not add a
Data-layer node either, and does not change any other node in this diagram.
See ["Header Bell — Second Surface Over the Same
Derivation"](#header-bell--second-surface-over-the-same-derivation-resolved-2026-09-05-prd-16-resolved-question-49-resolving-raise-traceability-matrixmd-gap-17)
in §14.
...
### Per-Priority SLA Target Hours — Confirmed As-Built (2026-09-08, PRD §16 Resolved Question 53)

The already-shipped **per-priority SLA target hours** for `RAISE-FR-MAINT-001`
are confirmed by business **as they stand**, resolving Open Finding **F-54**
(business chose "confirm as-is" from three presented options, over "supply
different values" and "decide MVP makes no SLA promise at all"):
...
This is a **confirmation of already-shipped behaviour, not a new design
decision** — no code change follows from it. As built:

- `frontend/src/services/ticket-service.ts:14` — `SLA_HOURS` map with the
  four values above.
- `go-template-main/service/ticketService.go:19-26` — the same map, mirroring
  the frontend map exactly.
- Every created ticket is stamped with `slaTargetHours` from this map, and the
  priority selector labels each choice with its SLA at the moment of filing
  ("Critical (2h SLA)", "High (8h SLA)", "Medium (24h SLA)", "Low (48h
  SLA)").

**Origin, recorded honestly:** these four values were inherited from the
ESAPS reference app (`src/data/requisitionData.ts`), which `CLAUDE.md`
classifies as REFERENCE ONLY — inheritance from it was never itself
authority, which is why this needed an explicit business confirmation before
being recorded here as confirmed. See PRD [§16 Resolved Question
53](../01-requirements/RAISE-PRD.md#16-open-questions) for the full record.

**This confirms one thing only: a single overall SLA target per ticket, keyed
by its priority.** It does **not** resolve, and must not be read as resolving,
"SLA per stage" below.

**Remaining TBD (per PRD §16 Q14, partially resolved by Resolved Question
53):** **SLA per stage** — how long each of the four workflow stages (User
Requisition → Dept Approval (Delegated) → IT Dispatch → Technician Execution)
may take — remains fully **OPEN**; no per-stage budget has been supplied by
business and none is implemented. The **vendor model** (internal technician
vs. external vendor dispatch), the **cost model**/tracking, and
**delegated-approver configuration rules** are likewise **not** defined in
the PRD and remain TBD in this design. This design does not invent values for
any of them.
...
layer as the necessary home for admin-configurable platform values, originally
driven by exactly one confirmed need — `RAISE-FR-WARRANTY-001`'s per-Asset-Category
"Expiring" threshold — and, **as of Design v0.15 (PRD v0.17, §16 Resolved
Question 46), a second confirmed need**: `RAISE-FR-EXEC-001`'s NBV KPI
useful-life value, following a comparable shape. **Re-keyed 2026-09-08
(Design v0.18, PRD v0.20, §16 Resolved Question 52), amending — without
deleting — that mechanism:** the NBV useful-life configuration key is
**per Asset Type** (`Asset.type`, the sub-category), not per Asset Category —
see [§13 Executive Intelligence, "NBV — Formula and Default Values Confirmed,
Not Yet Built"](#nbv--formula-and-default-values-confirmed-not-yet-built) for
the full business reasoning (why business could not give one fixed IT
Hardware figure, and why a per-Type table is a superset of a per-Category
one).
**Default values CONFIRMED 2026-09-23 (Design v0.20, PRD v0.22, §16 Resolved
Question 54), closing PRD §16 Open Question 3a:** business supplied one
useful-life value per Asset Type present in the data — see [§13 Executive
Intelligence, "NBV — Formula and Default Values Confirmed, Not Yet
Built"](#nbv--formula-and-default-values-confirmed-not-yet-built) for the full
ten-value table and the two business clarifications (Tablet is 5, not 3;
these ten are the only Asset Types in the data today, not a blanket default
for future/unlisted types — RQ51's unconfigured-lookup-key rule remains in
force unchanged for those). As of PRD v0.19 (§16 Resolved Questions 50–51),
the NBV tile's placement (tenth tile, both P-002 and P-014) and its behavior
for a lookup key with no configured useful life (returns `purchaseCost`
unchanged — unaffected by the Category→Type re-key, see Resolved Question 52)
were already confirmed. **With Resolved Question 54, every input this design
needs is now confirmed — the NBV tile is now buildable** (design shape,
formula, and the ten Asset-Type useful-life values all confirmed; nothing is
implemented yet).
...
Settings Domain — Application/Domain
        ...
        │
        ├── NBVSettings: Record<AssetType, usefulLifeYears>
        │       (one entry per Asset Type present in the data — re-keyed
        │        from Asset Category, PRD §16 Resolved Question 52; shape
        │        confirmed; ten default values CONFIRMED 2026-09-23, PRD §16
        │        Resolved Question 54 — see §13; now buildable, not yet built)
        │
        ▼
...
Consumed by: Warranty status computation
             ...
             NBV KPI computation, now buildable
             (see §13 "NBV — Formula and
             Default Values Confirmed, Not Yet Built")
```

- **Settings (Admin)** — an admin-facing UI surface for viewing and editing
  platform configuration values; for MVP, scoped to Warranty thresholds per
  Asset Category (built) and NBV useful-life per Asset **Type** (shape and
  ten default values confirmed, not yet built; re-keyed from Category, PRD
  §16 Resolved Question 52).
- **Settings Domain** — holds configuration state keyed by concept (currently
  `WarrantySettings`, keyed by Asset Category, plus the confirmed-but-not-yet-
  built `NBVSettings` shape, keyed by Asset **Type**, with its ten values now
  confirmed) and exposes it for read by other domains (Warranty status
  computation; NBV KPI computation, now buildable) and for admin-gated write.
...
`RAISE-FR-WARRANTY-001`'s threshold and, as of PRD v0.17 §16 Resolved Question
46 (values confirmed PRD v0.22 §16 Resolved Question 54), `RAISE-FR-EXEC-001`'s
NBV useful-life value. It should **not** be read as
authorizing a general-purpose, multi-section admin Settings framework beyond
these two confirmed drivers — any further Settings section (e.g., Alert
channel configuration, future License renewal-alert configuration per
[§5.3](#53-license-domain-enterprise-roadmap)) would need its own PRD
confirmation before being added here, the same discipline applied to every
other design area in this document.

### Dependencies

- `RAISE-FR-WARRANTY-001` (per-category Expiring threshold — built)
- `RAISE-FR-EXEC-001` (per-Asset-Type NBV useful-life value — shape
  confirmed 2026-09-05, PRD §16 Resolved Question 46; re-keyed from Category
  to Type 2026-09-08, PRD §16 Resolved Question 52; ten default numbers
  confirmed 2026-09-23, PRD §16 Resolved Question 54, closing Open Question
  3a; now buildable, not yet built)
- `RAISE-FR-ASSET-002` (Category & Hierarchy — the configuration key for
  Warranty; `Asset.type`, the sub-category, is the configuration key for
  NBV, PRD §16 Resolved Question 52)
...
those remain PRD-listed proposal KPIs (see "NBV/Risk/Utilization — KPI
Status" below). **As of 2026-09-05, this is no longer uniformly "not yet
implemented":** Utilization is now built and live (PRD v0.17, §16 Resolved
Question 48); NBV's formula is confirmed but still blocked on missing default
values (PRD v0.17, §16 Resolved Question 46, Open Question 3a); Risk is
confirmed out of MVP scope by business decision, not a gap (PRD v0.17, §16
...
(which render the same grid) — and a confirmed **behavior for a lookup key
with no configured useful life** (keeps `purchaseCost` unchanged; unaffected
by the Category→Type re-key below, PRD §16 Resolved Question 52). **Re-keyed
2026-09-08 (PRD v0.20, §16 Resolved Question 52), amending — without
deleting — Resolved Question 46's mechanism:** the useful-life configuration
key itself is amended from per Asset Category to per Asset **Type**
(`Asset.type`, the sub-category) — see "NBV — Formula and Default Values
Confirmed, Not Yet Built" below for the full reasoning.

**Further resolved 2026-09-23 (PRD v0.22, §16 Resolved Question 54), closing
[Open Question 3a](../01-requirements/RAISE-PRD.md#16-open-questions):**
business supplied the ten default useful-life values, one per Asset Type
present in the data. None of the resolutions above — the tile-placement/
unconfigured-key resolutions, the Category→Type re-key, or this final values
confirmation — change what has already shipped: NBV remains **NOT BUILT**.
What has changed is that it is **now buildable** — every input this design
needs (tenth-tile placement, formula, clamp-at-0 rule, per-Type configuration
key, unconfigured-key behavior, and now the ten numeric values themselves) is
confirmed. The grid below is therefore updated to its **specified and now
buildable** ten-tile shape, with the NBV tile clearly marked as
specified-and-buildable-but-not-built. See "NBV — Formula and Default Values
Confirmed, Not Yet Built" below for full detail.
...
│   (nine built and live; NBV specified and now buildable, not built)  │
├────────────┬────────────┬────────────┬──────────────┬───────────────┤
...
│            │ Question 50), formula CONFIRMED, ten default useful-life  │
│            │ values per Asset Type CONFIRMED (PRD §16 Resolved         │
│            │ Question 54, closing Open Question 3a) — NOW BUILDABLE,  │
│            │ still NOT BUILT. See "NBV — Formula and Default          │
│            │ Values Confirmed, Not Yet Built" below.                  │
├────────────┴────────────┴────────────────────────────────────────────┤
│ AI Insights                                                          │
│ AI Portfolio Health                                                  │
...
(tenth tile — placement, formula, and the ten per-Asset-Type default
useful-life values all confirmed [Open Question
3a](../01-requirements/RAISE-PRD.md#16-open-questions) closed by PRD §16
Resolved Question 54 — **now buildable, but still not built**; see below).
Business
...
Maintenance/Warranty/License data — this design does not invent one, with one
exception as of PR #102 (2026-09-05): the Utilization tile does have a
PRD-defined formula (see below), and this list documents that it is now
implemented against that formula. This list documents what exists; it is not
a new specification for further development.

## NBV/Risk/Utilization — KPI Status

The PRD identifies NBV, Risk, and Utilization as proposal-defined KPIs under
`RAISE-FR-EXEC-001`. As of PRD v0.17 (2026-09-05, §16 Resolved Questions
46–48), the three no longer share one status — each is recorded separately:

- **Utilization — BUILT AND LIVE** (PRD v0.17, §16 Resolved Question 48). See
  "Utilization — Built and Live" below.
- **NBV — formula, tile placement, and default useful-life values all
  CONFIRMED; NOW BUILDABLE, but NOT BUILT** (PRD v0.17 §16 Resolved Question
  46; tile placement PRD v0.19 §16 Resolved Question 50; unconfigured-lookup-
  key behavior PRD v0.19 §16 Resolved Question 51; configuration key re-keyed
  from per Asset Category to per Asset Type PRD v0.20 §16 Resolved Question
  52; ten default values, one per Asset Type, CONFIRMED PRD v0.22 §16
  Resolved Question 54, closing Open Question 3a). See "NBV — Formula and
  Default Values Confirmed, Not Yet Built" below.
- **Risk — CONFIRMED OUT OF MVP SCOPE for this dashboard, by business
  ...
  tile as a TBD/gap on `RAISE-FR-EXEC-001` is superseded by this. See "Risk —
  Confirmed Not MVP" below.

### Utilization — Built and Live

**Definition — Resolved 2026-08-21 (PRD v0.3, §16 Resolved Question 27):**
Business confirmed the definition as **assignment-time-based**:
...
**Built and live — 2026-09-05 (PRD v0.17, §16 Resolved Question 48):** this
required no new business decision — the definition (Resolved Question 27) and
mechanics (Resolved Question 29) were already resolved on 2026-08-21; it had
simply not been built yet. Implemented as `frontend/src/lib/utilization.ts`
plus the Executive Dashboard KPI tile, PR #102 (merged 2026-09-05, commit
`321265f`), live-verified reading **66.7% / "8 of 12 assignable assets"**
against the 15-asset seeded register.

**Derivation shape — a pure function, no new entity or history table:** the
real-time-snapshot ruling in Resolved Question 29 is precisely what makes this
possible. At a single instant, "share of available time spent assigned"
collapses to "share of eligible assets currently assigned" — a pure function
over the Available and Assigned counts the Executive Dashboard already
fetches for its own KPI tiles (see the As-Built KPI grid above), with **no
assignment-history table** required. This is consistent with this document's
[§2.3 Deterministic First](#23-deterministic-first) principle: no AI/LLM
involvement, a plain read-time computation.

**Implementation fact PRD §16 Resolved Question 29 could not have
anticipated:** Resolved Question 29 names Disposed, Retired, **and** Under
Maintenance as the three denominator-exclusion states, but the design's
`AssetStatus` model (see [§18 Logical Data
Model](#18-logical-data-model)) has only four values — `'Available' |
'Assigned' | 'In Maintenance' | 'Retired'` — and there is **no Disposed
status**, because Disposal is confirmed Enterprise Roadmap (PRD §16 Resolved
Question 26). So two of the three named exclusions apply today (Retired, In
Maintenance) and the third (Disposed) has nothing to exclude yet. **If a
Disposed status is ever added to the design's asset lifecycle model, it must
be added to this exclusion set too.**

### NBV — Formula and Default Values Confirmed, Not Yet Built

**Method — confirmed 2026-09-05 (PRD v0.17, §16 Resolved Question 46):
straight-line depreciation.**

```text
NBV = purchaseCost − (purchaseCost ÷ usefulLifeYears × assetAgeInYears)
NBV = max(NBV, 0)   // salvage/residual value is zero; clamped at 0
```

computed from the `purchaseDate` and `purchaseCost` fields that already exist
end-to-end on every Asset record (`frontend/src/types/asset.ts`,
`go-template-main/model/assetModel.go`, `sql/pg/V1__Assets_Table.sql`). **No
...
function takes a useful-life lookup as an **injected callback**
(`usefulLifeYearsFor: (category: string) => number`, called as
`usefulLifeYearsFor(asset.category)`) and **defines no default useful-life
values of its own** — it has **zero consumers**, and `NBVSettings` does not
exist anywhere in `frontend/src` (nothing wires this callback to a real
lookup yet). This design records the confirmed configuration *shape*; it does
not restate or duplicate the function's internals, and no NBV feature is
implemented.

**Re-keyed 2026-09-08 (PRD v0.20, §16 Resolved Question 52), amending —
without deleting — the "per Asset Category" mechanism confirmed 2026-09-05
(Resolved Question 46):** the NBV useful life is configured **per Asset
Type** (`Asset.type`, the sub-category), not per Asset Category. Asked to
supply the five per-Category default values, business could not give a single
fixed figure for IT Hardware — *"ไม่มีกำหนดตายตัว แล้วแต่อุปกรณ์ที่ซื้อ"* (no fixed
value, it depends on the equipment purchased) — while confirming the other
four categories can each carry one. Presented with four options — per Type
using the existing `type` field; per Asset with a new field; a Category
default plus a per-asset override; keeping per-Category and naming one
representative value for IT Hardware — business chose **per Type**. **Why
one uniform per-Type key, not a two-level model:** a per-Type table is a
**superset** of a per-Category table — for the four categories that do have a
fixed value, the same value simply repeats across that category's types,
exactly equivalent to a single category-level value — so one uniform
per-Type key satisfies both answers, with **no special case for IT
Hardware**. **No new field or data model is needed** — `Asset.type` already
exists end-to-end (`go-template-main/sql/pg/V1__Assets_Table.sql:7`,
`go-template-main/model/assetModel.go:23`, and the frontend `Asset` type and
its fixtures), the same precedent as the **F-27** resolution establishing
that `type` **is** the sub-category of `category`. **Implementation-fact gap
flagged, not acted on:** `frontend/src/lib/nbv.ts`'s injected callback is
currently written as `(category: string) => number`, called with
`asset.category` — matching the pre-Resolved-Question-52 mechanism; since the
function has zero consumers and no `NBVSettings` exists yet, no code is
"wrong" today, but a future implementation must call this lookup with
`asset.type`, not `asset.category`.

**Useful life — configurable per Asset Type**, set by an admin in Settings,
following the exact precedent already established by [§5.4 Settings
...
Question 41 — genuinely per-Category, unaffected by this re-key) — **not** a
single global constant. The intended shape is: one useful-life-in-years value
per Asset **Type present in the data** — not a fixed enumeration; `type` is
free-text (`varchar(100)`) and the set of values grows as assets are added,
exactly as `category` does. **The types currently present in seeded data**
(derived from `frontend/src/data/fixtures/mockData.ts`, not invented): IT
Hardware → Laptop, Monitor, Headphones; Mobile → Smartphone, Tablet; Office
Equipment → Printer, Projector; Infrastructure → Router, Server; Media
Equipment → Camera — recorded as `NBVSettings: Record<AssetType,
usefulLifeYears>` — see [§5.4 Settings Domain](#54-settings-domain) for this
design area's confirmed second driver; that shape is authoritative and is not
re-defined here. **This is not recorded as a fixed count of types** — the
requirement is one configured value per type present in the data, not a
fixed enumeration. **These are the same ten types for which business has now
supplied a confirmed useful-life value (PRD §16 Resolved Question 54) — see
"NBV for a lookup key with no configured useful life" and "Default
useful-life values — CONFIRMED" below** for the confirmed table and for why
this is not a blanket default for any Type added later.
...
**NBV for a lookup key with no configured useful life — CONFIRMED 2026-09-07
(PRD v0.19, §16 Resolved Question 51), generalizes unchanged to the
Category→Type re-key (2026-09-08, §16 Resolved Question 52):**
`computeAssetNbv` returns `purchaseCost` **unchanged** — the asset is treated
as **not yet depreciated**, and **stays included** in the portfolio total.
This is reachable by **data alone**, not only by misconfiguration: both
`AssetCategory` and `Asset.type` are open `string` types
...
already-implemented, test-pinned behavior** (recorded as **R-36** in
`OPEN-FINDINGS.md`) rather than requesting a change — **no code change
follows from it, either from Resolved Question 51 or from Resolved Question
52**. Alternatives considered and rejected: returning **0** (would assert the
asset is worthless); returning **NaN** (would render as the literal string
"NaN" on a KPI tile); **excluding** such assets from the portfolio total
(would silently understate the portfolio).

**Default useful-life values — CONFIRMED 2026-09-23 (PRD v0.22, §16 Resolved
Question 54), closing PRD §16 [Open Question
3a](../01-requirements/RAISE-PRD.md#16-open-questions):** business supplied
one useful-life value per Asset Type present in the data:
...
**Two points business clarified explicitly, both carried through as-given —
neither is inferable from the business's own raw phrasing ("5 years for
everything except mobile phones, 3 years"):**

1. **Tablet is 5, not 3.** Smartphone is the only Asset Type at 3 years;
   "mobile phone" in the business's phrasing meant the handset specifically,
   not the whole Mobile Category (which also contains Tablet).
2. **These ten are the only Asset Types present in the data today. This is
   explicitly NOT a blanket default for future/unlisted Asset Types.** A
   Type added later with no configured value continues to be governed
   unchanged by [§16 Resolved Question
   51](../01-requirements/RAISE-PRD.md#16-open-questions) above (returns
   `purchaseCost` unchanged, treated as not yet depreciated) — **this design
   does not introduce a default useful-life value for unlisted Asset Types**,
   and Resolved Question 54 does not ask for one.

**As of this sync, every input this design needs for the NBV tile is
confirmed** — tile placement, formula, clamp-at-0 rule, the per-Asset-Type
configuration key, unconfigured-key behavior, and now the ten default
values above. **The NBV tile is therefore now buildable. It remains NOT
BUILT** — this design records the confirmed inputs; it does not implement
the tile. Tracked as the resolution of Open Finding **F-03** in
...
as NBV — `go-template-main/service/assetService.go:101` sets `currentValue`
equal to `purchaseCost` on create and never recomputes it, and the seed
fixtures hold hand-written values at inconsistent rates. A future NBV tile
must be a newly derived read-time value (per the formula above), not a read
of `currentValue`.

### Risk — Confirmed Not MVP

**Confirmed 2026-09-05 (PRD v0.17, §16 Resolved Question 47): the Executive
Dashboard will NOT carry a Risk KPI tile for MVP.** This **confirms**, rather
than changes, what this design and the PRD already recorded:
`RAISE-AI-RISK-001` is Priority "Pilot (not confirmed MVP)" — see [§10 Risk
Scoring](#10-risk-scoring). PRD §16 [Open Question
4](../01-requirements/RAISE-PRD.md#16-open-questions)
("What is the exact definition of risk?") **remains open** but is **no longer
a blocker on `RAISE-FR-EXEC-001`'s MVP completeness** — it belongs entirely to
the Pilot/Roadmap `RAISE-AI-RISK-001` requirement, not to this P0 one.

**As-built observation — recorded and explicitly NOT a requirement, NOT a
specified design:** the shipped app already contains a "Portfolio Risk
Matrix" tab in the AI Decision Center, with a per-asset `riskScore`, a
Portfolio Risk Index, and risk bands. This decision leaves it exactly as-is —
it is not promoted into this design as an `RAISE-FR-EXEC-001` design area, and
it is not removed either. It is fixture-backed
(`frontend/src/data/fixtures/decisionData.ts` covers only 7 of 15 assets with
hand-written scores) and that one file uses three **mutually inconsistent**
risk-band thresholds (`>70`, `>75`/`>40`, and `80`/`60`/`30` at different
lines). This observation is tied to `RAISE-AI-RISK-001` (Pilot/Roadmap), not
to this Executive Dashboard section, so that nobody mistakes it for a
specified MVP feature of `RAISE-FR-EXEC-001`.
...
### Header Bell — Second Surface Over the Same Derivation (resolved 2026-09-05, PRD §16 Resolved Question 49, resolving `RAISE-TRACEABILITY-MATRIX.md` Gap 17)

**Two distinct artifacts, previously conflated.** For a month, both PRD §16
Resolved Question 35 (2026-08-21) and
`docs/project-foundation-baseline/ESAPS-UI-FOUNDATION-BASELINE.md` (same day)
used the name "NotificationCenter" to mean **different things**, which is
part of why the resulting contradiction was hard to see: Resolved Question 35
means the ESAPS reference page
`esaps_ai_template/src/pages/NotificationCenter.tsx` (103 lines, business/UI
reference only) — that page **remains entirely out of RAISE scope, no
Traceability ID at any tier, unchanged**, exactly as Resolved Question 35
already said (see [Out of Scope](#out-of-scope-no-design-area--by-business-decision)
below; the baseline document's row mapping that page to
`RAISE-FR-ALERT-001` as EXTEND was an error, since corrected in that file
separately, not here). The header bell in RAISE's own
`frontend/src/components/AppShell.tsx` is the **other** artifact: it was
built in RAISE, was never ported from `esaps_ai_template/`, and Resolved
Question 35 never referred to it. **This bell is confirmed in scope for
`RAISE-FR-ALERT-001`.**

**Same derivation, second surface — not a second Alert model.** The bell
does **not** introduce a second alert concept, a second severity model, or
any new stored record — it is a second display surface over the exact same
read-time derivation described in ["MVP — Read-Time Derivation, No New
Entity"](#mvp--read-time-derivation-no-new-entity) above. This is enforced
structurally, not just by convention: `deriveAlerts()`'s logic was extracted
into a shared hook, `frontend/src/hooks/useAlerts.ts`, and **both** the bell
and the Alerts screen (P-012) call that same hook. Because both surfaces
consume one shared derivation, they **cannot disagree** about what an alert
is, how many exist, or their order — there is no code path by which the bell
and P-012 could diverge on alert content, since both read the same output of
the same function. No Alert table, Alert entity, or persisted Alert record is
introduced by the bell any more than it already wasn't by the Alerts screen.

**What the bell displays.** The **first five** alerts in the ordering the
Alerts screen already uses — `deriveAlerts()`'s severity ordering, High →
Medium → Low, from the five trigger conditions above — plus a link to the
full Alerts screen at `ROUTES.NOTIFICATIONS` (`/notifications`).

**Why "first five in severity order," not "5 most recent" (recorded because
the obvious phrasing was rejected on evidence):** business's first request
was "the 5 most recent alerts." That phrasing is **not computable**: the
`Alert` interface (`frontend/src/lib/alerts.ts`) carries **no timestamp of
any kind**, because alerts are a read-time derivation with no persisted
record and therefore no creation time to sort by — `deriveAlerts()` sorts by
**severity** only. Business was shown this and confirmed the bell shows the
**first five of that same severity ordering**, so the bell and the Alerts
screen agree with each other by construction and no new business rule is
invented. Sorting by the underlying Asset/Ticket/Handover records' own dates
was explicitly considered and **explicitly rejected** — it would be a new
ordering rule with no existing definition, and none is introduced here.

**Badge count — total, not unread.** The bell's badge shows the **total**
number of alerts currently returned by the shared derivation. There is
deliberately **no unread/read state anywhere in this design**: alert
acknowledgement, dismissal, read/unread marking, and snooze all remain
**out of MVP scope**, exactly as before RQ49 — this resolution does not
touch any of them. The badge count is therefore a count of alerts that
**currently exist** (per the read-time derivation), not a count of alerts
**unseen** by the viewing user — there is no "seen" concept to count against.
This document must not be read as implying an unread-count semantic anywhere
the badge is described.

**Accessibility and dead-prop cleanup (as-built, recorded for completeness).**
The bell button previously had **no accessible name at all**; `aria-label`
and `aria-expanded` have been added to it. Separately, `AppShell`'s
`notifications` prop — which no caller ever populated (zero
`<AppShell ... notifications=` call sites) and which rendered a permanently
empty dropdown behind placeholder text reading "this will be wired to the
real API in a later phase" — has been **removed**. The bell now derives its
own alert list via the shared `useAlerts` hook rather than depending on a
prop no one ever passed.

**Still not decided by this resolution — unaffected, stays exactly as open
as before:** alert acknowledgement, dismissal, read/unread state, or snooze;
alert delivery, scheduling, or digesting; a notification-preference model;
and per-user filtering of which alerts a given user sees ([Open Question
22a](#25-design-open-questions)) — the bell shows the same alerts to every
authenticated user, exactly as the Alerts screen does today. PRD Open Finding
**F-03** (unrelated — Executive Dashboard NBV default values) is likewise
...
**Resolved 2026-09-05 (was: "Open, not resolved by this design"):** whether
the header bell-icon dropdown in `AppShell` is in scope for
`RAISE-FR-ALERT-001`, and whether that conflicted with PRD §16 Resolved
Question 35's "entirely out of RAISE scope" finding for the ESAPS reference
page `NotificationCenter.tsx`, was tracked here as an open, unreconciled
contradiction through Design v0.15. It is now resolved — see ["Header Bell —
Second Surface Over the Same
Derivation"](#header-bell--second-surface-over-the-same-derivation-resolved-2026-09-05-prd-16-resolved-question-49-resolving-raise-traceability-matrixmd-gap-17)
above (PRD §16 Resolved Question 49): the two artifacts are different
things — the ESAPS `NotificationCenter.tsx` page stays out of scope
unchanged (Resolved Question 35 stands), and RAISE's own `AppShell` bell is
confirmed in scope for `RAISE-FR-ALERT-001`. Nothing further remains open on
this specific question; per-user alert filtering ([Open Question
22a](#25-design-open-questions)) is a separate, still-open question,
untouched by this resolution.
...
- `NotificationCenter.tsx` (the ESAPS reference page,
  `esaps_ai_template/src/pages/NotificationCenter.tsx` — distinct from
  `RAISE-FR-ALERT-001` Alerts, and distinct from the header bell in RAISE's
  own `frontend/src/components/AppShell.tsx`, which **is** in scope for
  `RAISE-FR-ALERT-001` — see [§14, "Header Bell — Second Surface Over the
  Same Derivation"](#header-bell--second-surface-over-the-same-derivation-resolved-2026-09-05-prd-16-resolved-question-49-resolving-raise-traceability-matrixmd-gap-17),
  resolved 2026-09-05, PRD §16 Resolved Question 49, resolving
...
| RAISE-FR-EXEC-001 | Executive Dashboard (§13 — Utilization KPI **built and live** 2026-09-05, PR #102; NBV formula, tenth-tile placement (both P-002 and P-014), unconfigured-lookup-key behavior, the per-Asset-Type configuration key (re-keyed 2026-09-08 from per-Asset-Category, §16 Resolved Question 52), and the ten per-Asset-Type default useful-life values (confirmed 2026-09-23, §16 Resolved Question 54, closing Open Question 3a) all confirmed — **NBV is now buildable, still not built**; Risk KPI confirmed **NOT MVP** for this dashboard, by decision, not gap) / Settings (§5.4 — NBV useful-life configuration home, per Asset Type, shape and values both confirmed) |
...
Resolved Question 33), and updated again 2026-09-08 to record the already-shipped
per-priority SLA target hours as business-confirmed as-built (§16 Resolved Question
53) — a confirmation of shipped behaviour, not a workflow change, and one that leaves
"SLA per stage" fully OPEN. One PRD item, `RAISE-NFR-SEC-RBAC-001`, is intentionally not a
...
change any other row in this table. **As of Design v0.15 (PRD v0.17, §16
Resolved Questions 46–48):** the `RAISE-FR-EXEC-001` row above now records
three separate, no-longer-uniform statuses instead of one "TBD" — Utilization
built and live (Resolved Question 48), NBV formula confirmed but blocked on
missing default values (Resolved Question 46, Open Question 3a), and Risk
confirmed out of MVP scope by decision, not gap (Resolved Question 47). The
`RAISE-FR-EXEC-001` row now also cross-references [§5.4 Settings
Domain](#54-settings-domain) for the NBV useful-life configuration shape, the
same cross-reference pattern already used for the `RAISE-FR-WARRANTY-001`
row. No new PRD Traceability ID is introduced by this change — NBV useful-life
configuration is a design-layer extension of the existing Settings Domain,
the same treatment already given to the Warranty threshold. **As of Design
v0.16 (PRD v0.18, §16 Resolved Question 49):** the `RAISE-FR-ALERT-001` row
above now also covers the header bell in `frontend/src/components/AppShell.tsx`,
confirmed as a **second Presentation-layer surface** over the same read-time
derivation (shared via the new `frontend/src/hooks/useAlerts.ts` hook), not a
second Alert model or a new Traceability ID. This resolves
`RAISE-TRACEABILITY-MATRIX.md` Gap 17 and the contradiction the prior version
of this document (§14) recorded as still-open between PRD §16 Resolved
Question 35 and `docs/project-foundation-baseline/ESAPS-UI-FOUNDATION-BASELINE.md`.
The resolution reaffirms, rather than reopens, Resolved Question 35 itself:
the ESAPS reference page `esaps_ai_template/src/pages/NotificationCenter.tsx`
remains out of scope, no row, no design area, unchanged — see the [Out of
...
confirmed behavior for a lookup key with no configured useful life (returns
`purchaseCost` unchanged). Neither resolution changes NBV's overall status —
it remains **NOT BUILT and NOT YET BUILDABLE**, blocked solely on the
still-open [Open Question 3a](../01-requirements/RAISE-PRD.md#16-open-questions)
numeric values. No new PRD Traceability ID is introduced. **As of Design
v0.18 (PRD v0.20, §16 Resolved Question 52):** the `RAISE-FR-EXEC-001` row
above now also records that the NBV useful-life configuration key is
**re-keyed from per Asset Category to per Asset Type** (`Asset.type`, the
sub-category) — amending, without deleting, Resolved Question 46's "per
Asset Category" mechanism. [Open Question
3a](../01-requirements/RAISE-PRD.md#16-open-questions) is re-scoped
accordingly (one value per Asset Type present in the data, not five values
per Asset Category) and, at that point, remained fully **OPEN** — no numeric
value had yet been supplied or invented for any category or any type. No new
PRD Traceability ID was introduced. **As of Design v0.20 (PRD v0.22, §16
Resolved Question 54):** business supplied the ten confirmed default
useful-life values, closing Open Question 3a. The `RAISE-FR-EXEC-001` row
above is updated accordingly — NBV's status changes from "confirmed shape,
blocked on missing values" to **"all inputs confirmed, now buildable, still
not built."** No verdict on `RAISE-FR-EXEC-001` changes as a result — the
...
3. What is utilization? — **Resolved 2026-08-21** (PRD v0.3 §16 Resolved
   ...
   denominator exclusions. **Built and live 2026-09-05** (PRD v0.17, §16
   Resolved Question 48; see [§13 Executive
   Intelligence](#13-executive-intelligence), "Utilization — Built and
   Live"). **NBV formula narrowed 2026-09-05** (PRD v0.17, §16 Resolved
   Question 46): method confirmed (straight-line, zero salvage, clamped at
   0, configurable useful life via [§5.4 Settings
   Domain](#54-settings-domain)) — default useful-life values **now
   confirmed, see item 3a below (RESOLVED 2026-09-23)**. **Configuration key
   re-keyed 2026-09-08** (PRD v0.20, §16 Resolved Question 52, amending
   without deleting Resolved Question 46): the key is per Asset **Type**,
   not per Asset Category — see item 3a. **Risk KPI confirmed NOT MVP for the
   Executive Dashboard, 2026-09-05** (PRD v0.17, §16 Resolved Question 47) —
   see item 4 below, which is no longer a blocker on this requirement. **With
   item 3a now resolved, every input `RAISE-FR-EXEC-001`'s NBV tile needs is
   confirmed — the tile is now buildable, though still not built.**
3a. What are the default useful-life values **per Asset Type** (the
   sub-category, e.g. Laptop, Monitor, Smartphone — one value per `type`
   value present in the data, not a fixed enumeration) for NBV straight-line
   depreciation? — **RESOLVED 2026-09-23** (PRD v0.22, §16 Resolved Question
   54). Originally added 2026-09-05 alongside PRD v0.17 §16 Resolved Question
   46 (PRD §16 Open Question 3a), first raised as five values **per Asset
   Category** (IT Hardware, Mobile, Office Equipment, Infrastructure, Media
   Equipment), then **re-scoped 2026-09-08** (PRD v0.20, §16 Resolved
   Question 52, amending without deleting Resolved Question 46) to one value
   per Asset **Type** — business could not supply one fixed figure for IT
   Hardware ("depends on the equipment purchased"), confirmed the other four
   categories each carry one, and chose the per-Type option because a
   per-Type table is a superset of a per-Category one (see [§13 Executive
   Intelligence](#13-executive-intelligence), "NBV — Formula and Default
   Values Confirmed, Not Yet Built" for the full reasoning). **Business has
   now supplied all ten values** (Laptop 5, Monitor 5, Headphones 5,
   Smartphone 3, Tablet 5, Printer 5, Projector 5, Server 5, Router 5, Camera
   5 — years). Two clarifications carried through as business gave them, not
   inferred: Tablet is 5 (not 3 — Smartphone alone is the 3-year handset
   exception within Mobile); and these ten are the only Asset Types present
   in the data today, **not** a blanket default for any future/unlisted
   Type (those remain governed by [§16 Resolved Question
   51](../01-requirements/RAISE-PRD.md#16-open-questions), unaffected). **This
   item (3a) is now CLOSED** — resolved as the closure of Open Finding
...
4. What is risk? — **Confirmed 2026-09-05 to no longer block
   ...
   this question belongs entirely to `RAISE-AI-RISK-001` (already
   Pilot/Roadmap, not MVP — see [§10 Risk Scoring](#10-risk-scoring)), not to
   this P0 requirement. The exact definition of risk **remains open** for
   that Pilot/Roadmap requirement.
...
5a. What are the alert trigger conditions, severity model, and channel scope
   ...
   any authenticated user, any role — see Security item 22 below. **The
   header-bell scope contradiction is now also resolved, 2026-09-05** (PRD
   §16 Resolved Question 49, resolving `RAISE-TRACEABILITY-MATRIX.md` Gap
   17): the ESAPS reference page `NotificationCenter.tsx` stays out of scope
   (Resolved Question 35 reaffirmed, the baseline document's contrary row
   was an error), and RAISE's own `AppShell` header bell is a **different,
   in-scope artifact** — a second surface over the same shared alert
   derivation, showing the first 5 alerts in existing severity order plus a
   link to `ROUTES.NOTIFICATIONS` — see [§14, "Header Bell — Second Surface
   Over the Same
   Derivation"](#header-bell--second-surface-over-the-same-derivation-resolved-2026-09-05-prd-16-resolved-question-49-resolving-raise-traceability-matrixmd-gap-17).
   **Still open, not decided by any of these resolutions:** alert
   acknowledgement/dismissal/read-unread/snooze, alert
   delivery/scheduling/digesting, a notification-preference model, and
   per-user filtering of which alert rows a given user sees (item 22a
   below).
...
9. What fields are required for maintenance? — workflow **shape** confirmed
   ...
   [§5.1 Maintenance Domain](#51-maintenance-domain)). Already-shipped
   **per-priority SLA target hours** confirmed as-built **2026-09-08** (PRD
   §16 Resolved Question 53: Critical 2h, High 8h, Medium 24h, Low 48h — a
   confirmation of shipped behaviour, not a new decision). **SLA per stage**,
   vendor model, cost model, and delegated-approver configuration rules
   remain fully **open** — Resolved Question 53 confirms only a single
   overall SLA target per ticket, keyed by priority, and does not touch any
   of these four.
...
`RAISE-FR-MAINT-001` — Critical 2h, High 8h, Medium 24h, Low 48h — are
confirmed by business **as they stand**. Business was presented three
options (confirm as-is; supply different values; decide MVP makes no SLA
promise at all) and chose **confirm as-is**. **This confirms already-shipped
behaviour; no code change follows from it.** As built:
`frontend/src/services/ticket-service.ts:14` (`SLA_HOURS` map) and
`go-template-main/service/ticketService.go:19-26` (the same map, mirroring
the frontend map exactly) already stamp every created ticket with
`slaTargetHours`, and the priority selector already labels each choice with
its SLA (e.g. "Critical (2h SLA)"). **Origin recorded honestly:** these
values were inherited from the ESAPS reference app
(`src/data/requisitionData.ts`), which `CLAUDE.md` classifies REFERENCE
ONLY — inheritance was never itself authority, which is why business
confirmation was needed before this design could record the values as
confirmed, per Open Finding **F-54** (frontend/backend were already shipping
and displaying SLA commitments while the PRD listed SLA as TBD). **This
decision resolves only the single overall per-priority SLA target per
ticket — it does NOT resolve "SLA per stage."** [§5.1 Maintenance
Domain](#51-maintenance-domain) and [§25 Design Open Questions, item
9](#25-design-open-questions) both still list SLA **per stage** (how long
each of the four workflow stages may take), the vendor model, the cost
model, and delegated-approver configuration rules as fully **OPEN** — none
of these four is touched by this sync. The [§24 Design
Traceability](#24-design-traceability) `RAISE-FR-MAINT-001` verdict
(**PASS**, resting on the confirmed 4-stage transitions, §16 Resolved
Question 33, with SLA deliberately excluded as NOT TESTABLE YET so no
executed test asserts an SLA value) is **untouched**. **No numeric value
beyond the four confirmed above was written anywhere in this document by
this sync.**

**Version:** 0.18 (sync with PRD v0.19 → v0.20, §16 Resolved Question 52:
the NBV useful-life configuration key is amended from per Asset Category to
per Asset Type. This **amends, without deleting,** §16 Resolved Question 46 —
asked to supply the five per-Category default values, business stated IT
Hardware has no fixed figure (*"ไม่มีกำหนดตายตัว แล้วแต่อุปกรณ์ที่ซื้อ"* — depends on
the equipment purchased), while confirming the other four categories can
each carry one; presented with four options (per Type using the existing
`type` field; per Asset with a new field; a Category default plus a
per-asset override; keeping per-Category and naming one representative value
for IT Hardware), business chose **per Type**. A per-Type table is a
superset of a per-Category one — for the four categories that do have a
fixed value, the same value simply repeats across that category's types —
so one uniform per-Type key satisfies both answers with no special case for
IT Hardware. **No new field or data model is needed** — `Asset.type` already
exists end-to-end (`go-template-main/sql/pg/V1__Assets_Table.sql:7`,
`go-template-main/model/assetModel.go:23`, frontend `Asset` type/fixtures),
the same precedent as the **F-27** resolution establishing that `type`
**is** the sub-category of `category`. [§16 Open Question
3a](../01-requirements/RAISE-PRD.md#16-open-questions) is re-scoped
accordingly — one numeric value per Asset Type present in the data, not five
values per Asset Category — and **remains fully OPEN**; no numeric value has
been supplied or invented for any category or any type, anywhere in this
document. [§16 Resolved Question
51](../01-requirements/RAISE-PRD.md#16-open-questions)'s unconfigured-lookup-
key rule is **unaffected** and generalizes without needing an edit, since
`type` is free-text just as `category` is. This is a confirmed business
decision from a live chat session — `## NEEDS_PRD_CONFIRMATION` is **not**
raised by this sync.)

**Change Log — v0.19 → v0.20 (sync with PRD v0.21 → v0.22: NBV per-Asset-Type
default useful-life values confirmed, PRD §16 Resolved Question 54, closing
Open Question 3a):**

1. **§5.4 Settings Domain** — the "Unlike the Warranty threshold, the NBV
   useful-life values have no confirmed defaults yet" paragraph is replaced
   with a "Default values CONFIRMED 2026-09-23" paragraph, cross-referencing
   the new confirmed table in §13 and both business clarifications (Tablet is
   5, not 3; the ten values are not a blanket default for unlisted Types).
   The `NBVSettings` component/data-flow diagram, its bullet descriptions,
   the Scope Boundary paragraph, and the Dependencies list are all updated
   from "shape confirmed, default values NOT confirmed... not buildable yet"
   to "shape and ten default values confirmed... now buildable, not yet
   built."
2. **§13 Executive Intelligence** — the "NBV — Formula Confirmed, Default
   Values Still Missing" subsection is renamed **"NBV — Formula and Default
   Values Confirmed, Not Yet Built"** and its "Blocked" paragraph is replaced
   with the confirmed ten-value table (Laptop/Monitor/Headphones/Tablet/
   Printer/Projector/Server/Router/Camera = 5 years; Smartphone = 3 years)
   plus both business clarifications, stated verbatim as given. The Status
   Note, the KPI grid diagram's NBV tile cell, the "9 of 10 built" narrative
   paragraph, and the "NBV/Risk/Utilization — KPI Status" bullet are all
   updated from "NOT BUILT and NOT YET BUILDABLE" to "NOW BUILDABLE, still
   NOT BUILT." **Nothing is recorded as implemented** — no code reference is
   added or changed beyond what §13 already recorded before this sync.
3. **§24 Design Traceability** — the `RAISE-FR-EXEC-001` table row and the
   narrative paragraph following the Design v0.18 entry are both updated to
   record the ten confirmed values and the "now buildable" status. No
   verdict changes: `RAISE-FR-EXEC-001`'s Acceptance Criteria never asserted
   an NBV value (NBV was NOT TESTABLE YET pending exactly this input), so no
   pass/fail test result is affected by this sync.
4. **§25 Design Open Questions** — item 3a is marked **RESOLVED 2026-09-23**
   and closed, carrying the confirmed ten-value table and both business
   clarifications; item 3's summary line is updated to point to the
   resolution instead of "remain open."
5. **No value beyond the ten confirmed above (and no default for any
   unlisted Asset Type) was written anywhere in this document by this
   sync.** RQ46's formula, RQ52's per-Asset-Type keying, RQ51's
   unconfigured-Type behavior, and RQ50's tenth-tile/Monthly-Depreciation-
   unchanged decisions are all restated only as prior confirmed fact, not
   amended.
6. **No `## NEEDS_PRD_CONFIRMATION` signal raised by this design pass.**
   PRD §16 Resolved Question 54 is already a confirmed business decision; no
   new capability lacking a requirement was found during this pass.

**Change Log — v0.18 → v0.19 (sync with PRD v0.20 → v0.21: already-shipped
per-priority SLA target hours for `RAISE-FR-MAINT-001` confirmed as-built;
PRD §16 Resolved Question 53):**

1. **§5.1 Maintenance Domain** — new subsection "Per-Priority SLA Target
   Hours — Confirmed As-Built" added, recording the confirmed table
   (Critical 2h, High 8h, Medium 24h, Low 48h), the as-built code references
   (`frontend/src/services/ticket-service.ts:14`,
   `go-template-main/service/ticketService.go:19-26`), the ESAPS-inherited
   origin, and an explicit statement that only the single overall
   per-priority SLA target is confirmed — "SLA per stage" is not. The
   existing "Remaining TBD" paragraph is updated to make clear it is
   *partially* resolved (per-priority target only) and that SLA per stage,
   vendor model, cost model, and delegated-approver configuration rules all
   remain fully OPEN, unchanged.
2. **§24 Design Traceability** — `RAISE-FR-MAINT-001` row and cross-check
   paragraph updated to record the confirmed per-priority SLA target hours
   as business-confirmed as-built, with an explicit note that SLA per stage
   remains OPEN. **No verdict changed** — `RAISE-FR-MAINT-001` remains
   **PASS**, resting on the confirmed 4-stage transitions (§16 Resolved
   Question 33); the Acceptance Criteria deliberately excluded SLA as NOT
   TESTABLE YET, so no executed test asserts an SLA value, and this sync
   introduces none.
3. **§25 Design Open Questions** — Data item 9 updated to record the
   confirmed per-priority SLA target hours and to restate, explicitly, that
   SLA per stage, vendor model, cost model, and delegated-approver
   configuration rules remain fully open.
4. **No number beyond the four confirmed values (Critical 2h, High 8h,
   Medium 24h, Low 48h) was written anywhere in this document by this
   sync.** The vendor model, cost model, and delegated-approver
   configuration rules were left exactly as open as they were before this
   sync — business said nothing about them.
5. **No `## NEEDS_PRD_CONFIRMATION` signal raised by this design pass.**
   PRD §16 Resolved Question 53 is already business-confirmed; no new
   capability lacking a requirement was found during this pass.

**Change Log — v0.17 → v0.18 (sync with PRD v0.19 → v0.20: NBV useful-life
configuration key re-keyed from per Asset Category to per Asset Type; PRD §16
Resolved Question 52, amending §16 Resolved Question 46):**

1. **§5.4 Settings Domain** — intro paragraph, Conceptual Component & Data
   Flow diagram (`NBVSettings` re-keyed from `Record<AssetCategory,
   usefulLifeYears>` to `Record<AssetType, usefulLifeYears>`), component
   descriptions, and Dependencies all updated to record the per-Asset-Type
   key; the Warranty threshold (`WarrantySettings`, per-Category) is
   explicitly unchanged and untouched by this sync.
2. **§13 Executive Intelligence** — "NBV — Formula Confirmed, Default Values
   Still Missing" extended with a new "Re-keyed 2026-09-08" paragraph
   recording the full business reasoning (IT Hardware has no fixed figure;
   the four-option choice; the per-Type-is-a-superset-of-per-Category
   argument; no new field needed, per the F-27 precedent); the "Useful life
   — configurable per Asset Category" heading and content renamed to "per
   Asset Type," listing the ten types currently present in seeded data (not
   recorded as a fixed count); the "NBV for an Asset Category with no
   configured useful life" subsection renamed "NBV for a lookup key with no
   configured useful life" and updated to state Resolved Question 51's rule
   generalizes unchanged to Type; the "Blocked" paragraph updated to
   describe the outstanding numbers as one per Asset Type, not five per
   Asset Category; an implementation-fact note added recording that
   `frontend/src/lib/nbv.ts`'s injected callback is currently written and
   called against `asset.category`, not yet updated for this re-key (no code
   change follows from this design sync).
3. **§13 Status Note and KPI grid** — "Further narrowed 2026-09-07"
   paragraph extended with the 2026-09-08 re-key; the ten-tile grid's NBV
   row text updated from "five per-Asset-Category default useful-life
   values" to "one default useful-life value per Asset Type."
4. **§24 Design Traceability** — `RAISE-FR-EXEC-001` row and cross-check
   paragraph both updated to record the re-key; no new Traceability ID
   introduced.
5. **§25 Design Open Questions** — Business item 3 and item 3a both
   updated: item 3a now asks for one value per Asset Type present in the
   data (originally five values per Asset Category), remains fully OPEN.
6. **No useful-life number was written anywhere in this document by this
   sync** — every reference to the outstanding default values states only
   that they remain undefined (PRD §16 Open Question 3a), per PRD §16's
   explicit instruction not to invent or use an illustrative number as if
   confirmed, for any category or any type.
7. **Warranty's `WarrantySettings.expiringThresholdDaysByCategory`
   (PRD §16 Resolved Question 41) is untouched** — it is genuinely
   per-Category and is not re-keyed by this sync.
8. **No `## NEEDS_PRD_CONFIRMATION` signal raised by this design pass.** The
   re-key traces directly to PRD v0.20 §16 Resolved Question 52, already
   confirmed by business in a live chat session; no new capability lacking a
   requirement was found during this pass.
9. Header metadata updated: Version bumped to 0.18; Design Source updated to
   reference PRD v0.20.
...
the header bell in `frontend/src/components/AppShell.tsx` is in or out of
scope for `RAISE-FR-ALERT-001` — is resolved. Two distinct artifacts had
been conflated under the same "NotificationCenter" name: the ESAPS reference
page `esaps_ai_template/src/pages/NotificationCenter.tsx` stays entirely out
of RAISE scope (PRD §16 Resolved Question 35 reaffirmed; the
`ESAPS-UI-FOUNDATION-BASELINE.md` row mapping it to `RAISE-FR-ALERT-001` was
an error, corrected in that file separately); RAISE's own `AppShell` header
bell is confirmed **in scope** for `RAISE-FR-ALERT-001`, as a second display
surface over the same read-time alert derivation, sharing its logic with the
Alerts screen (P-012) via the new `frontend/src/hooks/useAlerts.ts` hook.
Bell shows the first 5 alerts in the existing severity ordering (High →
Medium → Low) plus a link to `ROUTES.NOTIFICATIONS`; badge shows the total
alert count, not an unread count (no unread/read state exists in MVP scope).
This is a confirmed business decision from a live chat session —
`## NEEDS_PRD_CONFIRMATION` is **not** raised by this sync. Documentation-only
sync; no design decision invents anything the PRD or the as-built code does
not already state. Per-user alert filtering (PRD §16 Open Question 22a) and
Open Finding F-03 are both untouched and remain exactly as open as before.)

**Change Log — v0.15 → v0.16 (sync with PRD v0.17 → v0.18: header bell
in/out-of-scope contradiction resolved; PRD §16 Resolved Question 49,
resolving `RAISE-TRACEABILITY-MATRIX.md` Gap 17):**

1. **§3.1 High-Level Architecture** — traceability note on the `A7` Alerts
   node extended: `A7` now also covers the `AppShell` header bell as a
   second Presentation-layer surface reading the same derivation via a
   shared hook; no new Data-layer node, no change to any other node.
2. **§14 Alert Architecture** — new subsection "Header Bell — Second Surface
   Over the Same Derivation" added, recording: the two-distinct-artifacts
   resolution (ESAPS `NotificationCenter.tsx` stays out of scope; RAISE's own
   `AppShell` bell is in scope); the shared `useAlerts` hook as a structural
   (not conventional) guarantee that the bell and Alerts screen cannot
   disagree; the first-five-by-severity display rule and its rejected
   "5 most recent" alternative (no timestamp field exists on `Alert`); the
   total-count (not unread-count) badge semantic, with acknowledge/dismiss/
   read-unread/snooze explicitly reaffirmed out of MVP scope; and the
   accessibility (`aria-label`/`aria-expanded`) and dead-prop-removal
   (`notifications` prop) as-built details. The prior "Open, not resolved by
   this design" bell paragraph is rewritten to point to this new subsection
   as resolved.
3. **§22 Out of Scope** — `NotificationCenter.tsx` bullet expanded to name
   the ESAPS page explicitly by path and cross-reference the new §14
   subsection, so the bullet no longer reads as if it also covered the
   `AppShell` bell.
4. **§24 Design Traceability** — `RAISE-FR-ALERT-001` row and cross-check
   paragraph both updated to record the bell resolution; no new
   Traceability ID introduced (the bell is a design-layer surface over an
   existing requirement, not a new requirement).
5. **§25 Design Open Questions** — Business item 5a updated: the header-bell
   scope contradiction is marked resolved and no longer listed among "still
   open" items; item 22a (per-user alert filtering) is carried forward
   unchanged, still open.
6. **No `## NEEDS_PRD_CONFIRMATION` signal raised by this design pass.**
   Every statement added traces directly to PRD v0.18 §16 Resolved Question
   49 or to already-implemented code (`frontend/src/hooks/useAlerts.ts`,
   `frontend/src/components/AppShell.tsx`, `frontend/src/lib/alerts.ts`) the
   PRD itself cites. No capability was found during this pass that lacks a
   requirement behind it.
7. Header metadata updated: Version bumped to 0.16; Design Source updated to
   reference PRD v0.18.

**Change Log — v0.14 → v0.15 (sync with PRD v0.16 → v0.17: NBV formula
confirmed but blocked on missing default values; Risk KPI confirmed NOT MVP
for the Executive Dashboard; Utilization KPI confirmed built and live; PRD
§16 Resolved Questions 46, 47, 48; new PRD §16 Open Question 3a):**

1. **§13 Executive Intelligence** — "Logical Dashboard — Current MVP (As
   Built)" KPI grid updated from 8 to 9 tiles, adding the built Utilization
   tile; "NBV/Risk/Utilization — Proposal KPIs, Not Yet Implemented" renamed
   to "NBV/Risk/Utilization — KPI Status" and restructured into three
   subsections reflecting each KPI's own status: "Utilization — Built and
   Live" (definition/mechanics restated, build/PR #102 details added,
   derivation-shape rationale added — a pure function over Available/
   Assigned counts, no assignment-history table, per the real-time-snapshot
   ruling in Resolved Question 29; the AssetStatus-has-no-Disposed-value
   implementation fact recorded), "NBV — Formula Confirmed, Default Values
   Still Missing" (formula, per-category useful-life mechanism, zero-salvage/
   clamp-at-0 rule, and the `currentValue`-cannot-serve-as-NBV supporting
   fact all recorded; explicitly not built and not yet buildable pending
   Open Question 3a), and "Risk — Confirmed Not MVP" (decision recorded as a
   decision, not a gap; the as-built Portfolio Risk Matrix tab recorded as an
   observation, explicitly not a requirement or specified design).
2. **§5.4 Settings Domain** — extended with a second confirmed driver: NBV
   per-Asset-Category useful-life configuration, following the exact shape
   already used for the Warranty Expiring threshold. Conceptual data-flow
   diagram, component descriptions, Scope Boundary, and Dependencies all
   updated. Explicitly records that only the *shape* is confirmed — no
   default useful-life numbers are supplied by this design.
3. **§24 Design Traceability** — `RAISE-FR-EXEC-001` row updated to record
   the three separate KPI statuses and a new cross-reference to §5.4;
   cross-check paragraph extended with a v0.15 note.
4. **§25 Design Open Questions** — Business item 3 updated (Utilization
   built; NBV narrowed; Risk resolved-not-a-blocker); new item 3a added
   (default useful-life values, explicitly not supplied); item 4 updated
   (risk definition remains open but no longer blocks `RAISE-FR-EXEC-001`).
5. **No `## NEEDS_PRD_CONFIRMATION` signal raised by this design pass.**
   Every statement added traces directly to PRD v0.17 §16 Resolved Questions
   46, 47, or 48, or to already-merged code (`frontend/src/lib/utilization.ts`,
   PR #102, commit `321265f`) the PRD itself cites. No capability was found
   during this pass that lacks a requirement behind it.
6. Header metadata updated: Version bumped to 0.15; Design Source updated to
   reference PRD v0.17.
