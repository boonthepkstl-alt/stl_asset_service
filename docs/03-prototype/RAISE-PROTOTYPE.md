...
§16 Open Question 3a):** `RAISE-PRD.md` v0.22 §16 Resolved Question 54 and
`RAISE-DESIGN.md` v0.20 §13/§5.4 record business's answer to Decision Request DR-02:
the ten per-Asset-Type default useful-life values for `RAISE-FR-EXEC-001`'s NBV KPI —
the last missing input identified by Resolved Question 52's re-key. Confirmed values:
Laptop, Monitor, Headphones, Tablet, Printer, Projector, Server, Router, and Camera
each **5 years**; Smartphone **3 years** — the only Asset Type at 3 years. Two
clarifications business gave explicitly, neither inferable from the raw phrasing ("5
years for everything except mobile phones, 3 years"), are carried through unchanged:
**(1) Tablet is 5, not 3** — "mobile phone" meant the Smartphone handset specifically,
not the whole Mobile Category; **(2) these ten are the only Asset Types present in the
data today — explicitly NOT a blanket default for any future/unlisted Asset Type**,
which continues to be governed unchanged by Resolved Question 51 (an unconfigured
lookup key returns `purchaseCost` unchanged, asset stays included in the portfolio
total, treated as not yet depreciated). **This is a values supply, not new scope:**
Resolved Question 52's per-Asset-Type keying, Resolved Question 50's tenth-tile
placement, Resolved Question 46's straight-line/zero-salvage/clamped-at-0 formula, and
Resolved Question 51's unconfigured-key behavior are all unaffected and restated
nowhere as new or amended by this pass. **Concrete edits in this pass:** **[§23A P-018
Settings](#23a-p-018-settings)**'s NBV wireframe — previously showing each of the ten
Asset-Type rows as `[ — no default value confirmed — ]` — now shows the confirmed
figures (`5` for nine types, `3` for Smartphone), and its prose notes are updated from
"no default values exist... none is invented here" to record the closure of Open
Question 3a; the section's Status Banner, Purpose, and Traceability lines are updated
from "shape confirmed only" to "shape and default values confirmed," while **the
section itself remains explicitly NOT BUILT** — no `NBVSettings` type, service,
repository, or UI code exists in `frontend/src/` today, and this pass does not claim
otherwise. **[§8 P-002 Main Dashboard](#8-p-002-main-dashboard)** and **[§20 P-014
Executive Dashboard](#20-p-014-executive-dashboard)**'s KPI grid diagrams and
"NBV/Risk/Utilization — Status" subsections are updated: the NBV tile's framing changes
from "NOT built and NOT yet buildable, blocked on PRD §16 Open Question 3a" to "NOT
built but NOW BUILDABLE — every confirmed input (placement, formula, per-Asset-Type
configuration key, unconfigured-key behavior, and now the ten default values) is in
place; only the build itself remains outstanding." **[§5 Screen
Inventory](#5-screen-inventory)** and **[§27 Prototype Traceability
Matrix](#27-prototype-traceability-matrix)**'s P-014 and P-018 rows are updated to
match — no row is added or removed, and no traceability verdict changes (Open Finding
F-03 is narrowed further, not closed — the tile itself is still not built).
`RAISE-PRD.md` and `RAISE-DESIGN.md` are not modified by this pass; no `##
NEEDS_PRD_CONFIRMATION` signal is raised — Resolved Question 54 is already a confirmed
business decision, dated 2026-09-23, answered via Decision Request DR-02. No value
beyond the ten confirmed by business is invented, and no default is introduced for any
Asset Type outside those ten. See the "Document Status" section's Change Log for full
detail.
...
`RAISE-PRD.md` v0.20 §16 Resolved Question 52 and `RAISE-DESIGN.md` v0.18 §13 confirm
that the NBV useful-life configuration recorded against `RAISE-FR-EXEC-001` (Resolved
Question 46, 2026-09-05) is **re-keyed from Asset Category to Asset Type** — this is a
**re-key, not new scope**: the configuration driver, the tile it feeds, and every other
already-confirmed NBV decision (formula, placement, unconfigured-lookup behavior) are
unchanged. **Why:** business was asked to supply the five per-Category default
useful-life values (PRD §16 Open Question 3a) and answered that **IT Hardware has no
fixed value** — "it depends on the equipment purchased" — while confirming the other
four categories can each carry one; offered four options (per Type using the existing
`type` field; per Asset with a new field; a Category default plus per-asset override;
keeping per-Category with one representative value), business chose **per Type**.
**Why one uniform Type key, not a two-level Category-default-plus-Type-exception
model:** a per-Type table is a **superset** of a per-Category table — for the four
categories that do have a fixed value, the same value simply repeats across that
category's types, exactly equivalent to a single category-level value; **no special
case for IT Hardware is needed**. **No new field is required** — `Asset.type` already
exists end to end (`go-template-main/sql/pg/V1__Assets_Table.sql:7` `type varchar(100)
NOT NULL`, `go-template-main/model/assetModel.go:23` `Type string`,
`frontend/src/types/asset.ts:30` `type: string`, plus `frontend/src/data/fixtures/
mockData.ts`) — same precedent as the Open Finding F-27 resolution this document
already records: `type` **is** the sub-category of `category` (see [§11 P-005 Category
& Hierarchy](#11-p-005-category--hierarchy), unaffected by this pass). **Concrete
edits in this pass:** the two occurrences of `NBVSettings: Record<AssetCategory,
usefulLifeYears>` in **[§23A P-018 Settings](#23a-p-018-settings)** are re-keyed to
`Record<AssetType, usefulLifeYears>`; that section's wireframe now lists one row per
Asset **Type** currently seeded — IT Hardware → Laptop, Monitor, Headphones; Mobile →
Smartphone, Tablet; Office Equipment → Printer, Projector; Infrastructure → Router,
Server; Media Equipment → Camera — derived from `frontend/src/data/fixtures/
mockData.ts`, **not invented**, and **not recorded as a fixed count**: `type` is a
free-text field and this set grows as assets are added, exactly as `category` already
does (per F-27). **[§8 P-002 Main Dashboard](#8-p-002-main-dashboard)** and **[§20
P-014 Executive Dashboard](#20-p-014-executive-dashboard)**'s "NBV/Risk/Utilization —
Status" subsections and **[§27 Prototype Traceability Matrix](#27-prototype-
traceability-matrix)**'s P-014/P-018 rows are updated to say "per Asset Type" rather
than "per Asset Category" for NBV only. Of the roughly 41 "per-Asset-Category"/"per
category" phrases in this document, only those describing the **NBV useful-life
configuration** are changed; every phrase describing **Warranty's per-Category
"Expiring" threshold** (Resolved Question 41, genuinely per-Category, untouched),
**P-005's Category → Type grouping/hierarchy content**, and **Utilization's Resolved
Question 29 rules** is deliberately left as-is. **RQ51 is unaffected in substance:** a
lookup key with no configured useful life still yields `purchaseCost` unchanged and
stays included in the portfolio total — because `type` is free-text just as `category`
is, this rule generalizes to a newly-added type with no edit required. **PRD §16 Open
Question 3a remains fully OPEN**, re-scoped to ask for **one numeric value per Asset
Type** — **no illustrative or placeholder useful-life number is invented anywhere in
this pass**, including in the re-keyed wireframe. **This does not reopen the ten-tile
decision** — Resolved Question 50 stands unchanged (NBV is the tenth tile on P-002/
P-014; the illustrative "Monthly Depreciation" tile is kept unchanged); it does not
touch Resolved Question 51's behavior rule itself, only the key it operates on; and it
does not change any traceability verdict — `RAISE-FR-EXEC-001`'s status in every
matrix stays exactly as narrow/partial as before this pass. `RAISE-PRD.md` and
`RAISE-DESIGN.md` are not modified by this pass; Open Finding **F-03 stays OPEN**,
narrowed not closed; no `## NEEDS_PRD_CONFIRMATION` signal is raised (Resolved Question
52 is already business-confirmed, dated 2026-09-08). **Separately, this pass also
corrects a dating slip inherited from the prior sync:** the note below (originally
labelled "2026-09-07 re-sync") documents work that was **actually performed
2026-09-08** — corrected in place; every date in that note and elsewhere in this
document that correctly states **when a business decision was confirmed** (e.g.
Resolved Questions 50/51, 2026-09-07; Resolved Questions 46/47/48, 2026-09-05) is left
untouched. See the "Document Status" section's Change Log for full detail.

**Version note (2026-09-08 re-sync, v0.17 → v0.18, PRD §16 Resolved Questions 50/51 /
Design §13 "NBV — Formula Confirmed, Default Values Still Missing" further narrowed):**
`RAISE-PRD.md` v0.19 §16 Resolved Questions 50 and 51 and `RAISE-DESIGN.md` v0.17 §13
jointly confirm two further business decisions about `RAISE-FR-EXEC-001`'s NBV KPI,
narrowing (not closing) Open Finding F-03: **(1) NBV KPI tile placement is confirmed**
(Resolved Question 50) — the NBV KPI is specified as a **tenth tile** on the KPI grid,
on **both** **[§8 P-002 Main Dashboard](#8-p-002-main-dashboard)** and **[§20 P-014
Executive Dashboard](#20-p-014-executive-dashboard)** (they render the same grid); the
existing "Monthly Depreciation" tile is **kept unchanged** — not removed, not replaced,
not relabelled, not re-pointed at real data; business explicitly accepted that a real
computed NBV figure will sit directly beside a fabricated illustrative depreciation
figure on the same grid. Every place in P-002/P-014 that previously stated "no NBV tile
exists... and none can be built" is corrected: the tile **is now specified**; it remains
**NOT built and NOT yet buildable** only because the five default useful-life values are
still missing (Open Question 3a), not because the tile itself is undefined. **(2) NBV
for an Asset Category with no configured useful life is confirmed** (Resolved Question
51) — `computeAssetNbv` returns the asset's `purchaseCost` unchanged (not yet
depreciated); this confirms already-implemented, test-pinned behavior (R-36 in
`OPEN-FINDINGS.md`) and requires no code change; reachable by data alone because
`AssetCategory` is an open `string` type, not an enum of the five. **[§23A P-018
Settings](#23a-p-018-settings)**'s NBV section is updated with this behavior note; its
own status (shape confirmed only, NOT built) is unchanged. **PRD §16 Open Question 3a
(the five per-Asset-Category default useful-life values) remains fully OPEN — no
illustrative or placeholder number is invented anywhere in this pass.** **[§27 Prototype
Traceability Matrix](#27-prototype-traceability-matrix)**'s P-014 and P-018 rows are
updated to match. Every existing traceability reference back to `RAISE-FR-EXEC-001` is
kept intact. `RAISE-PRD.md` and `RAISE-DESIGN.md` are not modified by this pass; Open
Finding **F-03 stays OPEN**, narrowed not closed; no `## NEEDS_PRD_CONFIRMATION` signal is
raised (both decisions are already business-confirmed, dated 2026-09-07). See the
"Document Status" section's Change Log for full detail.

**Version note (2026-09-05 re-sync, v0.16 → v0.17, PRD §16 Resolved Question 49 /
Design §14 "Header Bell — Second Surface Over the Same Derivation," resolving
`RAISE-TRACEABILITY-MATRIX.md` Gap 17):** `RAISE-PRD.md` v0.18 §16 Resolved Question 49
and `RAISE-DESIGN.md` v0.16 §14 jointly confirm a business decision that closes the
in-scope/out-of-scope contradiction previously left open on **[§18 P-012
Alerts](#18-p-012-alerts)**: (1) PRD §16 Resolved Question 35 **stands unchanged** — the
ESAPS reference page `esaps_ai_template/src/pages/NotificationCenter.tsx` remains
**entirely out of RAISE scope**, no Traceability ID at any tier; the
`ESAPS-UI-FOUNDATION-BASELINE.md` row that had mapped that page to `RAISE-FR-ALERT-001`
as EXTEND was an **error**, already corrected in that file to DO NOT USE. (2) **The
header bell in RAISE's own `frontend/src/components/AppShell.tsx` is a different
artifact and IS in scope for `RAISE-FR-ALERT-001`** — it was built in RAISE, never
ported from `esaps_ai_template/`, and is now wired to the existing shared alert
derivation (`frontend/src/hooks/useAlerts.ts`, consumed by both the bell and P-012, so
the two surfaces cannot disagree on alert content). (3) **The bell shows the first five
alerts** in the same severity ordering (High → Medium → Low) already used by P-012, plus
a "View all alerts" link to `/notifications` — live-verified: badge reads **19** against
the seeded register, the five rows are `AST-0003 · iPhone 15 Pro` through `AST-0007 ·
Cisco Catalyst 9300` (all High), and are identical to P-012's own first five rows. The
badge counts alerts that **currently exist** (the shared derivation's total), not an
unread count — there is deliberately **no** acknowledge/dismiss/read-unread/snooze
affordance, unchanged by this resolution. "5 most recent" was business's first phrasing
but was rejected on evidence (`Alert` has no timestamp of any kind; `deriveAlerts()`
sorts by severity only) — business was shown this and confirmed the first five of that
same severity ordering; sorting by the underlying records' own dates was explicitly
considered and rejected. **[§18 P-012 Alerts](#18-p-012-alerts)** and **[§6 Global
Layout](#6-global-layout)** are both updated accordingly. Every existing traceability
reference back to `RAISE-FR-ALERT-001` is kept intact. PRD §16 Q22a (per-user alert
filtering) is unaffected and stays open. `RAISE-PRD.md` and `RAISE-DESIGN.md` are not
modified by this pass; no `## NEEDS_PRD_CONFIRMATION` signal is raised — this decision
is already business-confirmed, dated 2026-09-05. See the "Document Status" section's
Change Log for full detail.
...
| P-012 | Alerts | P0 | RAISE-FR-ALERT-001 — five MVP trigger conditions and fixed-per-condition severity confirmed 2026-09-04; access gate confirmed 2026-09-04 as any authenticated user (all four roles), enforced per-route in code; `AppShell` header bell confirmed 2026-09-05 as a second display surface over this same requirement (PRD §16 Resolved Question 49) — see §6 Global Layout |
...
| P-018 | Settings (Warranty Thresholds + NBV Useful Life) | P0, admin-only | RAISE-FR-WARRANTY-001 (built), RAISE-FR-EXEC-001 (NBV useful life — shape confirmed 2026-09-05, re-keyed per Asset Type 2026-09-08 [PRD §16 Resolved Question 52], ten default values confirmed 2026-09-23 [PRD §16 Resolved Question 54, closing Open Question 3a] — now buildable, not yet built) (Design §4.1B / §5.4 Settings Domain) |
...
```text
┌──────────────────────────────────────────────────────────────┐
│ RAISE                    Search / AI Search   🔔19   User    │
├───────────────┬──────────────────────────────────────────────┤
│               │                                              │
│ Dashboard     │                                              │
│ Assets        │                Main Content                   │
...
## Header Bell (`AppShell`) — Second Surface of P-012 Alerts, AS BUILT

**Resolved 2026-09-05, PRD §16 Resolved Question 49; Design §14 "Header Bell — Second
Surface Over the Same Derivation," resolving `RAISE-TRACEABILITY-MATRIX.md` Gap 17.**
This is global chrome, not a separate screen ID — it appears in the header on every
authenticated page, built in `frontend/src/components/AppShell.tsx`, and is described
here (rather than as its own `P-0xx` entry) because it has no independent purpose beyond
surfacing the same alert derivation used by **[§18 P-012 Alerts](#18-p-012-alerts)**. It
is a **different artifact** from the ESAPS reference page
`esaps_ai_template/src/pages/NotificationCenter.tsx` — that page stays entirely out of
RAISE scope (PRD §16 Resolved Question 35, unchanged); this bell was built independently
in RAISE and was never ported from it.

**Elements (as built, live-verified against the seeded register):**

- A bell button in the global header, badge showing the **total** alert count currently
  returned by the shared derivation — live-verified reading **19**. This is a count of
  alerts that **currently exist**, not an unread/unseen count — there is no read/unread
  concept anywhere in this design (see Design §14). Do not read the badge as implying a
  read/unread model.
- `aria-label` (e.g. "Notifications, 19 alerts") and `aria-expanded` on the button —
  previously the button had **no accessible name at all**; this is now fixed.
- Clicking opens a dropdown panel headed "Notifications" listing **exactly five** rows —
  the first five alerts in the same severity ordering (High → Medium → Low) that
  **[§18 P-012 Alerts](#18-p-012-alerts)** uses, drawn from the same shared hook
  (`frontend/src/hooks/useAlerts.ts`) so the two surfaces cannot disagree on alert
  content, count, or order. Each row shows the condition label, the description, and the
  affected record as `CODE · Name` (live-verified: `AST-0003 · iPhone 15 Pro` through
  `AST-0007 · Cisco Catalyst 9300`, all High severity in this dataset), with a severity
  dot coloured to match P-012's own severity badge variants.
- A **"View all alerts"** control at the foot of the panel navigates to
  `ROUTES.NOTIFICATIONS` (`/notifications`) — i.e. **[§18 P-012 Alerts](#18-p-012-alerts)**
  itself. Live-verified: the Alerts screen's first rows are identical to the bell's.

**Why "first five in severity order," not "5 most recent":** recorded once, in full, at
**[§18 P-012 Alerts](#18-p-012-alerts) → "Ordering Rationale"** below — not repeated
here to avoid drift between two copies of the same rationale.

**Deliberately absent, unchanged by this resolution:** acknowledge, dismiss, read/unread,
and snooze affordances; multi-channel delivery (Email/Teams/LINE Notify remain Phase 2 /
Enterprise Roadmap); a notification-preference model; per-user filtering of which alerts
a given user sees (PRD §16 Open Question 22a — separate, still open, untouched by this
resolution).

**Traceability:** `RAISE-FR-ALERT-001` — same requirement as P-012, confirmed as a
second display surface over the same read-time derivation, resolved 2026-09-05 (PRD §16
Resolved Question 49; Design §14).
...
## Sections (Specified: Ten Tiles) — Updated 2026-09-23, NBV tile's default useful-life values confirmed, tile now buildable (PRD §16 Resolved Question 54)

**Nine of the ten KPI tiles are built and live** (as of PR #102 / commit
`321265f`, merged 2026-09-05); **the tenth (NBV) is specified — placement,
formula, and default useful-life values all confirmed — and is now
BUILDABLE, but is still NOT built** (see "NBV/Risk/Utilization — Status"
below).
...
│            │kept unchanged)│ by PRD §16 Resolved Question 50)       │
├────────────┴────────────┴───────────────────────────────────────────┤
│    NBV     │ TENTH TILE — placement CONFIRMED (PRD §16 Resolved      │
│            │ Question 50), formula CONFIRMED, per-Asset-Type default │
│            │ useful-life values CONFIRMED (PRD §16 Resolved          │
│            │ Question 54, closing Open Question 3a) — NOW BUILDABLE, │
│            │ still NOT BUILT. See "NBV/Risk/Utilization —            │
│            │ Status" below.                                          │
├────────────┴────────────┴───────────────────────────────────────────┤
│ AI Insights                                                          │
│ AI Portfolio Health                                                  │
...
(2026-09-07); 9 of the 10 are built and live as of PR #102 / commit
`321265f`, merged 2026-09-05 (previously 8):** Total Assets, Available,
Assigned, **Utilization**, In Maintenance, Expired Warranty, Software
Licenses, Monthly Depreciation (illustrative — no depreciation model has been
built; **kept unchanged** by this decision — not removed, not replaced, not
relabelled, not re-pointed at real data), Monthly Cost (illustrative, same
caveat), and **NBV** (tenth tile — placement, formula, and the ten
per-Asset-Type default useful-life values all confirmed [PRD §16 Resolved
Question 54, closing Open Question 3a] — **now buildable, but still not
built**; see below). Business explicitly accepted that, once built, a real
computed NBV figure will sit directly beside the fabricated illustrative
Monthly Depreciation figure on the same grid.
...
## NBV/Risk/Utilization — Status as of 2026-09-23 (PRD v0.22 §16 Resolved Questions 46–48, 50–52, 54)

The PRD identifies NBV, Risk, and Utilization as proposal-defined KPIs under
`RAISE-FR-EXEC-001`. As of 2026-09-08, the three no longer share one status —
...
stays **OPEN**, narrowed further by this update, not closed):
...
- **NBV — placement, formula, and default useful-life values all CONFIRMED —
  NOW BUILDABLE, but still NOT built (PRD §16 Resolved Questions 46, 50, 51,
  52, 54).** Straight-line depreciation, computed from the existing
  `purchaseDate`/`purchaseCost` fields already on the Asset record — no new
  field or data model needed. Useful life is configurable **per Asset Type**
  via the new NBV section of **P-018 Settings** — re-keyed from Asset
  Category 2026-09-08 (PRD §16 Resolved Question 52, amending Resolved
  Question 46; see [§23A P-018 Settings](#23a-p-018-settings)); salvage value
  is zero; NBV is clamped at 0. **Tile placement — confirmed 2026-09-07 (PRD
  §16 Resolved Question 50):** the NBV KPI is specified as a **tenth tile**
  on this KPI grid, on both this screen and P-014 Executive Dashboard (they
  render the same grid). The existing "Monthly Depreciation" tile is **kept
  unchanged** — not removed, not replaced, not relabelled, and not
  re-pointed at real data by this decision; it stays static and explicitly
  illustrative, and business explicitly accepted that a real computed NBV
  figure will sit directly beside it. **Behavior for an Asset Type with no
  configured useful life — confirmed 2026-09-07 (PRD §16 Resolved Question
  51), re-keyed from Category to Type 2026-09-08 (PRD §16 Resolved Question
  52):** the computation returns the asset's `purchaseCost` unchanged,
  treating the asset as not yet depreciated (see "NBV for an Asset Type with
  No Configured Useful Life" in [§23A P-018 Settings](#23a-p-018-settings)
  for full detail). **Default useful-life values — CONFIRMED 2026-09-23 (PRD
  §16 Resolved Question 54), closing PRD §16 [Open Question
  3a](../01-requirements/RAISE-PRD.md#16-open-questions):** business supplied
  one useful-life value per Asset Type present in the data — 5 years for
  Laptop, Monitor, Headphones, Tablet, Printer, Projector, Server, Router,
  and Camera; 3 years for Smartphone, the only Asset Type at 3 years (see
  [§23A P-018 Settings](#23a-p-018-settings) for the full table and both
  business clarifications). **The tile itself is specified and now
  buildable — every confirmed input (placement, formula, per-Asset-Type
  configuration key, unconfigured-key behavior, and now the ten default
  values) is in place — but it is still NOT built.** This entry does not
  invent any value beyond the ten business confirmed, and introduces no
  default for any Asset Type outside those ten — an unlisted Type continues
  to be governed unchanged by Resolved Question 51 above.
...
## Status Note — Updated 2026-09-05: Header Bell Contradiction Resolved (PRD §16 Resolved Question 49; Design §14 "Header Bell — Second Surface Over the Same Derivation")

**This entry previously left the header bell-icon dropdown (`AppShell`) as an
unreconciled open question** — PRD §16 Resolved Question 35 said it was out of scope
entirely while `ESAPS-UI-FOUNDATION-BASELINE.md` mapped it to `RAISE-FR-ALERT-001` as
EXTEND. This is now resolved: those two statements were about **two different
artifacts**. Resolved Question 35 refers only to the ESAPS reference page
`esaps_ai_template/src/pages/NotificationCenter.tsx`, which stays entirely out of RAISE
scope, unchanged. The header bell in RAISE's own `frontend/src/components/AppShell.tsx`
is a separate artifact, never ported from `esaps_ai_template/`, and is confirmed **in
scope** for `RAISE-FR-ALERT-001` — it is a second display surface over the exact same
read-time derivation this screen already uses (both consume
`frontend/src/hooks/useAlerts.ts`), not a second Alert model. See **[§6 Global Layout →
"Header Bell"](#header-bell-appshell--second-surface-of-p-012-alerts-as-built)** for the
full as-built description, and "Ordering Rationale" below for why both surfaces share
"first five in severity order" rather than "5 most recent." The `ESAPS-UI-FOUNDATION-BASELINE.md` row is corrected separately, not here.
...
## Ordering Rationale — "First Five in Severity Order," Not "5 Most Recent" (resolved 2026-09-05, PRD §16 Resolved Question 49; Design §14)

Recorded once here because both this screen and the header bell (**[§6 Global
Layout](#header-bell-appshell--second-surface-of-p-012-alerts-as-built)**) share it.
Business's first request, when the header bell was scoped, was "the 5 most recent
alerts." That phrasing is **not computable**: the `Alert` interface
(`frontend/src/lib/alerts.ts`) carries **no timestamp field of any kind**, because
alerts are a read-time derivation with no persisted record and therefore no creation
time to sort by — `deriveAlerts()` sorts by **severity** only (High → Medium → Low, the
same ordering this screen already used before the bell existed). Business was shown this
and confirmed the bell displays the **first five of that same severity ordering**, so
the bell and this screen agree with each other by construction — both read the output of
the same shared hook (`frontend/src/hooks/useAlerts.ts`) and cannot diverge on content,
count, or order. Sorting by the underlying Asset/Ticket/Handover records' own dates was
explicitly considered and **explicitly rejected** — it would be a new ordering rule with
no existing definition, and none is introduced here.

## Open Questions — Left Open, Not Decided Here

- ~~**Header bell-icon dropdown (`NotificationCenter.tsx` in `AppShell`) — still
  unreconciled.**~~ **Resolved 2026-09-05** (PRD §16 Resolved Question 49; Design §14
  "Header Bell — Second Surface Over the Same Derivation," resolving
  `RAISE-TRACEABILITY-MATRIX.md` Gap 17) — the apparent contradiction was between two
  different artifacts, not a real conflict: PRD §16 Resolved Question 35 (out of scope)
  refers only to the ESAPS reference page `esaps_ai_template/src/pages/NotificationCenter.tsx`,
  unchanged; the `AppShell` header bell is a separate, RAISE-built artifact confirmed
  **in scope** for `RAISE-FR-ALERT-001` as a second surface over this screen's own
  derivation. See the Status Note at the top of this entry and **[§6 Global Layout →
  "Header Bell"](#header-bell-appshell--second-surface-of-p-012-alerts-as-built)** for
...
only. **The `AppShell` header bell (see [§6 Global Layout](#header-bell-appshell--second-surface-of-p-012-alerts-as-built))
is a second display surface over this same requirement, confirmed 2026-09-05 (PRD §16
Resolved Question 49; Design §14 "Header Bell — Second Surface Over the Same
Derivation") — not a separate requirement, not a separate screen ID, and not a second
Alert model.**
...
- **NBV — placement, formula, and default useful-life values all CONFIRMED —
  ...
  is zero; NBV is clamped at 0. **Tile placement — confirmed 2026-09-07 (PRD
  §16 Resolved Question 50):** the NBV KPI is specified as a **tenth tile**
  on this KPI grid, on both this screen and P-002 Main Dashboard (they
  render the same grid). The existing "Monthly Depreciation" tile is **kept
  unchanged** — not removed, not replaced, not relabelled, and not
  re-pointed at real data by this decision; it stays static and explicitly
  illustrative, and business explicitly accepted that a real computed NBV
...
(now shape- and default-value-confirmed, still not yet built) for a second
configuration driver: `RAISE-FR-EXEC-001`'s NBV useful-life value — re-keyed
2026-09-08 from per-Asset-Category to per-Asset-Type (PRD §16 Resolved Question 52,
amending Resolved Question 46; Design v0.18 §13), with its ten default values
confirmed 2026-09-23 (PRD §16 Resolved Question 54, closing Open Question 3a; Design
v0.20 §13/§5.4).** Lettered `23A`
...
section (added to this spec 2026-09-05; its default values confirmed 2026-09-23, PRD
§16 Resolved Question 54) is **confirmed in shape and default values, and is NOW
BUILDABLE, but is still NOT built** — no `NBVSettings` type, service, repository, or UI
section exists in `frontend/src/` today; do not read this spec as describing shipped
code for NBV.
...
(**built**), and the NBV useful-life value, per Asset Type — re-keyed from Asset
Category 2026-09-08, PRD §16 Resolved Question 52, with its ten default values
confirmed 2026-09-23, PRD §16 Resolved Question 54 (**shape and default values
confirmed, now buildable, still not yet built**). **Not a general Settings
framework** — see "Scope Boundary" below.
...
NBV — "Useful life (years) for straight-line depreciation" (NOT BUILT — shape and
     default values CONFIRMED, 2026-09-23, PRD §16 Resolved Question 54)
   — keyed by Asset TYPE, not Category (re-keyed 2026-09-08, PRD §16 Resolved
     Question 52); one row per Type currently present in the data, not a fixed count
──────────────────────────────────────────────────────────
...
### NBV section — shape and default values confirmed, NOW BUILDABLE, still NOT built (added 2026-09-05, PRD §16 Resolved Question 46; Design v0.15 §5.4; KPI-tile placement and unconfigured-lookup-key behavior further confirmed 2026-09-07, PRD §16 Resolved Questions 50–51; Design v0.17 §13; **re-keyed from Asset Category to Asset Type 2026-09-08, PRD §16 Resolved Question 52, amending Resolved Question 46; Design v0.18 §13**; **ten default useful-life values confirmed 2026-09-23, PRD §16 Resolved Question 54, closing Open Question 3a; Design v0.20 §13**)

- **Same confirmed pattern as the Warranty section above, but keyed by Asset Type, not
  Category (re-keyed 2026-09-08, PRD §16 Resolved Question 52):** one row per current
  Asset **Type**, admin-editable, following the exact same shape already established
  for the Warranty threshold (`WarrantySettings.expiringThresholdDaysByCategory`) — a
  per-type `usefulLifeYears` value (Design's `NBVSettings: Record<AssetType,
  usefulLifeYears>`). **This spec records the shape only; it does not describe existing
  code.**
- **Why per Type, not per Category — and why this is not a two-level model.** Business
  was asked to supply the five per-Category default useful-life values and answered
  that **IT Hardware has no fixed value** ("it depends on the equipment purchased"),
  while confirming the other four categories can each carry one; of four options
  offered (per Type using the existing `type` field; per Asset with a new field; a
  Category default plus a per-asset override; keeping per-Category with one
  representative value), business chose **per Type**. A per-Type table is a
  **superset** of a per-Category table: for the four categories that do have a fixed
  value, the same value simply repeats across that category's types, exactly
  equivalent to a single category-level value — **no special case for IT Hardware is
  needed**, and no two-level (Category-default-plus-Type-exception) model is required.
- **No new field required.** `Asset.type` already exists end to end
  (`go-template-main/sql/pg/V1__Assets_Table.sql:7` `type varchar(100) NOT NULL`,
  `go-template-main/model/assetModel.go:23` `Type string`,
  `frontend/src/types/asset.ts:30` `type: string`) — the same precedent as the Open
  Finding F-27 resolution this document already records: `type` **is** the sub-category
  of `category` (see [§11 P-005 Category & Hierarchy](#11-p-005-category--hierarchy)).
- **One row per Asset Type currently present in the data — not a fixed count.** The
  wireframe above shows the ten types currently seeded (`frontend/src/data/fixtures/
  mockData.ts`): IT Hardware → Laptop, Monitor, Headphones; Mobile → Smartphone,
  Tablet; Office Equipment → Printer, Projector; Infrastructure → Router, Server; Media
  Equipment → Camera. This is **not recorded as a rule of "ten types"** — `type` is a
  free-text field (like `category`), and this set grows as assets of a new type are
  added, exactly as P-005's Category → Type grouping already does.
- **Default values CONFIRMED 2026-09-23 (PRD §16 Resolved Question 54), closing PRD §16
  [Open Question 3a](../01-requirements/RAISE-PRD.md#16-open-questions):** business
  answered Decision Request DR-02 with one useful-life value per Asset Type present in
  the data — the wireframe above now shows the confirmed figures (5 years for Laptop,
  Monitor, Headphones, Tablet, Printer, Projector, Server, Router, and Camera; 3 years
  for Smartphone, the only Asset Type at 3 years). **Two clarifications business gave
  explicitly, carried through as-given, neither inferable from the raw phrasing** ("5
  years for everything except mobile phones, 3 years"): **(1) Tablet is 5, not 3** —
  "mobile phone" in that phrasing meant the Smartphone handset specifically, not the
  whole Mobile Category (which also contains Tablet); **(2) these ten are the only
  Asset Types present in the data today — this is explicitly NOT a blanket default for
  any future/unlisted Asset Type.** A Type added later with no configured row continues
  to be governed unchanged by Resolved Question 51 below (returns `purchaseCost`
  unchanged, treated as not yet depreciated) — **no default useful-life value is
  introduced here for unlisted Asset Types**, and Resolved Question 54 does not ask for
  one.
- **Not implemented anywhere in `frontend/src/`** — no `NBVSettings` type, no service/
  repository, no UI section. **This section is now buildable — every input it needs
  (shape, per-Asset-Type key, and the ten default values) is confirmed — but it remains
  NOT built**; this spec records the confirmed inputs, it does not implement the
  section (see [§8 P-002 Main Dashboard](#8-p-002-main-dashboard) / [§20 P-014
  Executive Dashboard](#20-p-014-executive-dashboard), "NBV/Risk/Utilization — Status,"
  for the same now-buildable-not-built framing as it affects the NBV KPI tile).
...
#### NBV for an Asset Type with No Configured Useful Life — confirmed 2026-09-07 (PRD §16 Resolved Question 51); key re-scoped from Category to Type 2026-09-08 (PRD §16 Resolved Question 52)

**RQ51 is unaffected in substance by the RQ52 re-key** — only the lookup key changes
(Category → Type); the behavior itself, its rationale, and its rejected alternatives
below are exactly as originally confirmed.

- **Behavior:** if an Asset's Type has no row configured in this section (no
  `usefulLifeYears` value), `computeAssetNbv` returns the asset's **`purchaseCost`
  unchanged** — the asset is treated as **not yet depreciated**, and it **stays
  included** in the portfolio NBV total. This is **not** a new decision requiring a
  code change — it **confirms already-implemented, test-pinned behavior** in
  `frontend/src/lib/nbv.ts`, recorded as **R-36** in `OPEN-FINDINGS.md`.
- **Reachable by data alone, not only by misconfiguration:** `Asset.type`
  (`frontend/src/types/asset.ts:30`) is declared as a free-text `string`, **not** an
  enum of the ten types shown in the wireframe above, so an Asset Type added later (or
  any value outside the currently-seeded types) carries no configured row here and
  falls into this behavior automatically — exactly as `AssetCategory`
  (`frontend/src/types/asset.ts:17`) already does for Category, per Open Finding F-27.
  This rule **generalized to the new Type key with no code or rule change** because
  `type` is free-text just as `category` is.
- **Alternatives put to business and rejected:** returning **0** (would assert the
  asset is worthless); returning **NaN** (would render as the literal string "NaN" on
  the NBV KPI tile); **excluding** such assets from the portfolio NBV total (would
  silently understate the portfolio).
- **No default useful-life number is invented by this rule** — it governs only the
  case where a type has *no* configured value; it does not supply one. PRD §16 Open
...
(Warranty per-category thresholds, built; NBV per-Asset-**Type** useful life, shape
confirmed 2026-09-05, re-keyed from Category to Type 2026-09-08 [PRD §16 Resolved
Question 52], ten default values confirmed 2026-09-23 [PRD §16 Resolved Question 54],
now buildable, not yet built). No other configuration area (e.g., alert rules,
...
Settings
      ...
      │
      └── NBV section (shape and default values confirmed, NOW BUILDABLE, still NOT
               built — keyed by Asset Type, re-keyed from Category 2026-09-08; ten
               default values confirmed 2026-09-23, PRD §16 Resolved Question 54)
               │
               └── (once built) Edit a type's useful-life value (pre-populated with
                    its confirmed default) → Save Changes → NBVSettings updated →
...
`RAISE-FR-EXEC-001` (shape- and default-value-confirmed, now-buildable-but-not-yet-built
driver of the NBV section, per PRD §16 Resolved Question 46 / Design v0.15 §5.4;
KPI-tile placement and unconfigured-lookup-key behavior further confirmed per PRD §16
Resolved Questions 50–51 / Design v0.17 §13; configuration key re-keyed from Asset
Category to Asset Type 2026-09-08, PRD §16 Resolved Question 52, amending Resolved
Question 46 / Design v0.18 §13; **ten default useful-life values confirmed 2026-09-23,
PRD §16 Resolved Question 54, closing Open Question 3a / Design v0.20 §13**) —
Design §4.1B/§5.4 explicitly frame this screen as design-layer only (not its own PRD
Traceability ID). Also depends on `RAISE-FR-ASSET-002` — **Category**, as the Warranty
section's configuration key, and **Type** (the sub-category of Category, per Open
Finding F-27), as the NBV section's configuration key — and `RAISE-NFR-SEC-RBAC-001`
(admin-only access, MVP enforcement level UI-only per PRD §16 Resolved Question 38).
See PRD §16 Resolved Questions 41/46/50/51/52/54 and Design §4.1B/§5.4/§13 for the full
...
| P-014 Executive | RAISE-FR-EXEC-001 | Planned — corrected 2026-08-31 to match as-built dashboard (Open Finding F-22); same built page as P-002 Main Dashboard. **Updated 2026-09-05 (PRD §16 Resolved Questions 46–48):** Utilization KPI now **built and live**; NBV formula confirmed but **blocked, not built** on missing per-Asset-Category default useful-life values (Open Question 3a); Risk KPI **confirmed out of MVP scope by business decision** (not a gap) — Open Finding F-03 narrowed, not closed. **Further updated 2026-09-08 (PRD §16 Resolved Questions 50–51):** NBV KPI tile placement confirmed as a **tenth tile** on the KPI grid (grid specified as 9 built + 1 specified-not-built = 10 tiles, both P-002 and P-014); Monthly Depreciation tile kept unchanged; NBV-for-unconfigured-lookup-key behavior confirmed (returns `purchaseCost` unchanged). **Further re-keyed 2026-09-08 (PRD §16 Resolved Question 52, amending Resolved Question 46):** the NBV useful-life configuration driving this tile is keyed by **Asset Type, not Category** — no change to tile placement, formula, or unconfigured-lookup behavior. **Further updated 2026-09-23 (PRD §16 Resolved Question 54, closing Open Question 3a):** the ten per-Asset-Type default useful-life values are now confirmed (5 years for nine types, 3 years for Smartphone) — the NBV tile is **now buildable, but still NOT built**; Open Finding F-03 narrowed further, not closed |
...
| P-018 Settings | RAISE-FR-WARRANTY-001, RAISE-FR-EXEC-001 (Design §4.1B/§5.4 — not itself a separate PRD Traceability ID) | Warranty section Implemented — new 2026-09-01 (PRD §16 Resolved Question 41; Design §4.1B/§5.4): admin-only Warranty section, one editable per-Asset-Category threshold, default 90 days, Save Changes / Reset. **NBV section added 2026-09-05 (PRD §16 Resolved Question 46; Design v0.15 §5.4) — shape confirmed only, NOT built:** per-Asset-Category useful-life value, blocked on missing default numbers (Open Question 3a). **Further confirmed 2026-09-07 (PRD §16 Resolved Questions 50–51; Design v0.17 §13):** the driven NBV KPI tile's placement (tenth tile, both P-002/P-014) and the unconfigured-lookup-key behavior (returns `purchaseCost` unchanged, confirming already-implemented `frontend/src/lib/nbv.ts`/R-36) are now both confirmed. **Re-keyed 2026-09-08 (PRD §16 Resolved Question 52, amending Resolved Question 46; Design v0.18 §13):** the NBV configuration key changes from Asset **Category** to Asset **Type** — one row per type present in the data (currently the ten seeded types), not a fixed count; Warranty's per-Category threshold is unaffected. **Default values confirmed 2026-09-23 (PRD §16 Resolved Question 54, closing Open Question 3a; Design v0.20 §13/§5.4):** business supplied all ten per-Asset-Type useful-life numbers (5 years for Laptop/Monitor/Headphones/Tablet/Printer/Projector/Server/Router/Camera, 3 years for Smartphone) — the wireframe now shows these confirmed figures instead of unset inputs. **This section's build status changes from "shape confirmed only" to "shape and default values confirmed, now buildable" — it remains NOT built**; no `NBVSettings` type/service/repository/UI exists in `frontend/src/` |
...
**v0.21 re-sync (2026-09-23, against PRD v0.22 §16 Resolved Question 54 / Design v0.20
§13/§5.4, closing PRD §16 Open Question 3a):** `RAISE-FR-EXEC-001`'s P-014 row and
P-018's row are both **updated, not added** — business supplied the ten confirmed
per-Asset-Type default useful-life values (5 years for Laptop, Monitor, Headphones,
Tablet, Printer, Projector, Server, Router, and Camera; 3 years for Smartphone),
answering Decision Request DR-02. This is a **values supply, not new scope**: the
per-Asset-Type keying (Resolved Question 52), the tenth-tile placement (Resolved
Question 50), the straight-line/zero-salvage/clamped-at-0 formula (Resolved Question
46), and the unconfigured-key behavior (Resolved Question 51) are all unaffected. The
NBV KPI tile and the P-018 NBV section both move from "specified/shape confirmed, NOT
built and NOT yet buildable" to "specified/shape and default values confirmed, NOW
BUILDABLE, still NOT built" — no code exists yet for either. Open Finding F-03 is
narrowed further by this pass, not closed. No row was added or removed and no new
requirement ID was introduced.
...
1. **Root confirmation.** PRD v0.22 §16 Resolved Question 54 and Design v0.20 §13/§5.4
   jointly record business's answer to Decision Request DR-02: the ten per-Asset-Type
   default useful-life values for `RAISE-FR-EXEC-001`'s NBV KPI, closing PRD §16 Open
   Question 3a — the last missing input identified by Resolved Question 52's re-key.
   Business answered DR-02 verbatim in Thai — "ใช้ 5 ปีทุกอย่างยกเว้นมือถือ 3 ปี" ("use 5
   years for everything except mobile phones, 3 years"). Confirmed table:
   ...
   Two clarifications business gave explicitly, carried through as-given, neither
   inferable from the raw phrasing above: **(a) Tablet is 5, not 3** — "mobile phone"
   meant the Smartphone handset specifically, not the whole Mobile Category (which also
   contains Tablet); **(b) these ten are the only Asset Types present in the data
   today — explicitly NOT a blanket default for any future/unlisted Asset Type**, which
   continues to be governed unchanged by Resolved Question 51 (unconfigured lookup key
   returns `purchaseCost` unchanged, asset stays included in the portfolio total,
   treated as not yet depreciated). **This is a values supply, not new scope:**
   Resolved Question 52's per-Asset-Type keying, Resolved Question 50's tenth-tile
   placement, Resolved Question 46's straight-line/zero-salvage/clamped-at-0 formula,
   and Resolved Question 51's unconfigured-key behavior are all unaffected and are not
   restated as new or amended by this pass.
2. **[§23A P-018 Settings](#23a-p-018-settings)** — the NBV wireframe's ten Asset-Type
   rows, previously each showing `[ — no default value confirmed — ]`, now show the
   confirmed figures (`5` for nine types, `3` for Smartphone). The section's Status
   Banner, Purpose, NBV-section heading, "no default values" bullet, Scope Boundary,
   User Flow, and Traceability lines are all updated from "shape confirmed only, NOT
   built" to "shape and default values confirmed, NOW BUILDABLE, still NOT built" — no
   `NBVSettings` type, service, repository, or UI code exists in `frontend/src/` today,
   and this pass does not claim otherwise. The "NBV for an Asset Type with No
   Configured Useful Life" subsection (Resolved Question 51) is **unchanged in
   substance** — it governs only the unlisted-Type case, which this pass does not
   touch.
3. **[§8 P-002 Main Dashboard](#8-p-002-main-dashboard)** and **[§20 P-014 Executive
   Dashboard](#20-p-014-executive-dashboard)** — the KPI grid ASCII diagram's NBV tile
   note and the "NBV/Risk/Utilization — Status" subsection's NBV bullet are both
   updated: framing changes from "NOT built and NOT yet buildable, blocked on PRD §16
   Open Question 3a" to "NOT built but NOW BUILDABLE — every confirmed input
   (placement, formula, per-Asset-Type configuration key, unconfigured-key behavior,
   and now the ten default values) is in place; only the build itself remains
   outstanding." The existing "Monthly Depreciation" tile is explicitly **not**
   affected — it stays static and illustrative, per Resolved Question 50, restated
   nowhere as changed by this pass.
4. **[§5 Screen Inventory](#5-screen-inventory)** and **[§27 Prototype Traceability
   Matrix](#27-prototype-traceability-matrix)**'s P-014 and P-018 rows are updated to
   match. No row is added or removed, and no requirement ID is introduced.
5. **No traceability verdict changes.** Open Finding F-03 is narrowed further by this
   pass (NBV now buildable, not just specified), not closed — the tile and the P-018
   NBV section remain NOT built. `RAISE-TRACEABILITY-MATRIX.md` is not itself edited by
   this pass (synced separately, downstream of this document).
6. **`RAISE-PRD.md` and `RAISE-DESIGN.md` are not modified by this pass.** No `##
   NEEDS_PRD_CONFIRMATION` signal is raised — Resolved Question 54 is already a
   confirmed business decision, dated 2026-09-23, answered via Decision Request DR-02.
   No value beyond the ten confirmed by business is invented anywhere in this pass, and
   no default is introduced for any Asset Type outside those ten.
7. Header metadata updated: Version bumped to 0.21; PRD Source updated to v0.22
   (advanced from v0.21); Design Source updated to v0.20 (advanced from v0.19), with
   the new §13 "NBV — Formula and Default Values Confirmed, Not Yet Built" framing
   added to the cited section list.
...
1. **Root confirmation.** PRD v0.20 §16 Resolved Question 52 and Design v0.18 §13
   confirm that the NBV useful-life configuration (Resolved Question 46, 2026-09-05)
   is **re-keyed from Asset Category to Asset Type** — a re-key, not new scope. Business
   was asked to supply the five per-Category default useful-life values and answered
   that **IT Hardware has no fixed value** ("it depends on the equipment purchased"),
   while confirming the other four categories can each carry one; of four options
   offered (per Type using the existing `type` field; per Asset with a new field; a
   Category default plus a per-asset override; keeping per-Category with one
   representative value), business chose **per Type**. A per-Type table is a
   **superset** of a per-Category table — the four categories that do have a fixed
   value simply repeat it across that category's types — so **no special case for IT
   Hardware is needed** and no two-level model is introduced. **No new field is
   required** — `Asset.type` already exists end to end
   (`go-template-main/sql/pg/V1__Assets_Table.sql:7`,
   `go-template-main/model/assetModel.go:23`, `frontend/src/types/asset.ts:30`), the
   same precedent as Open Finding F-27 (`type` is the sub-category of `category`).
2. **[§5 Screen Inventory](#5-screen-inventory)** — P-018's row updated to note the
   re-key.
3. **[§8 P-002 Main Dashboard](#8-p-002-main-dashboard)** and **[§20 P-014 Executive
   Dashboard](#20-p-014-executive-dashboard)** — the "NBV/Risk/Utilization — Status"
   subsection's NBV bullet is updated from "per Asset Category" to "per Asset Type";
   the KPI grid ASCII diagram's NBV tile note is updated to match. No change to tile
   placement, count, formula, or the unconfigured-lookup-key behavior itself (Resolved
   Questions 46/50/51 stand; only the key changes).
4. **[§23A P-018 Settings](#23a-p-018-settings)** — the two occurrences of
   `NBVSettings: Record<AssetCategory, usefulLifeYears>` are re-keyed to
   `Record<AssetType, usefulLifeYears>`; the wireframe's five-Category rows are
   replaced with one row per Asset Type currently seeded (`frontend/src/data/fixtures/
   mockData.ts`: Laptop, Monitor, Headphones, Smartphone, Tablet, Printer, Projector,
   Router, Server, Camera) — **not recorded as a fixed count**, since `type` is
   free-text and this set grows as assets are added, exactly as `category` already
   does. A new bullet explains the superset reasoning (why one uniform Type key, not a
   two-level model). The "NBV for an Asset Category with No Configured Useful Life"
   subsection is renamed "...Asset Type..."; RQ51's behavior itself is noted as
   unaffected in substance — only the lookup key changes. Purpose, Scope Boundary,
   User Flow, and Traceability are all updated to say "per Asset Type." The section's
   own build status is unchanged — still shape confirmed only, NOT built.
5. **[§27 Prototype Traceability Matrix](#27-prototype-traceability-matrix)**'s P-014
   and P-018 rows are updated to record the re-key. No row was added or removed.
6. **Reviewed, left unchanged (deliberately):** Warranty's per-Category "Expiring"
   threshold (Resolved Question 41) — genuinely per-Category; **[§11 P-005 Category &
   Hierarchy](#11-p-005-category--hierarchy)** and all Asset-Category
   grouping/distribution content; Utilization's Resolved Question 29 rules. Of the
   roughly 41 "per-Asset-Category"/"per category" phrases in this document, only those
   describing the NBV useful-life configuration were changed.
7. **Every existing traceability reference back to `RAISE-FR-EXEC-001` is kept intact.**
   PRD §16 Open Question 3a **remains fully OPEN**, re-scoped to ask for one numeric
   value per Asset Type — no illustrative or placeholder useful-life number is
   invented, suggested, or used as an example anywhere in this pass, including in the
   re-keyed wireframe. Open Finding **F-03 stays OPEN**, narrowed not closed. This does
   **not** reopen the ten-tile decision (Resolved Question 50 stands) and does **not**
   change any traceability verdict. No other screen, requirement ID, or row is touched
   by this pass.
8. **`RAISE-PRD.md` and `RAISE-DESIGN.md` are not modified by this pass.** No
   `## NEEDS_PRD_CONFIRMATION` signal is raised — Resolved Question 52 is already
   business-confirmed, dated 2026-09-08.
9. **Dating-slip correction (unrelated to the re-key above):** the prior "Change Log —
   v0.17 → v0.18" entry below, and the corresponding top-of-document Version note, were
   labelled "2026-09-07 re-sync" but that sync was **actually performed 2026-09-08** —
   corrected in place below. Every date that correctly states **when a business
   decision was confirmed** (e.g. Resolved Questions 50/51, 2026-09-07; Resolved
   Questions 46/47/48, 2026-09-05) is left untouched.
10. Header metadata updated: Version bumped to 0.19; PRD Source updated to v0.20
    (advanced from v0.19); Design Source updated to v0.18 (advanced from v0.17), with
    the updated §13 "NBV — Re-keyed to Asset Type" framing added to the cited section
    list.

**Change Log — v0.17 → v0.18 (2026-09-08, PRD §16 Resolved Questions 50/51 / Design §13,
per explicit business confirmation via direct chat session):**

1. **Root confirmation.** PRD v0.19 §16 Resolved Questions 50 and 51 and Design v0.17 §13
   jointly confirm two further business decisions about `RAISE-FR-EXEC-001`'s NBV KPI,
   narrowing (not closing) Open Finding F-03: **(a) NBV KPI tile placement confirmed**
   (Resolved Question 50) — the NBV KPI is specified as a **tenth tile** on the KPI grid,
   on **both** P-002 Main Dashboard and P-014 Executive Dashboard (they render the same
   grid); the existing "Monthly Depreciation" tile is **kept unchanged** — not removed,
   not replaced, not relabelled, not re-pointed at real data; business explicitly accepted
   that a real computed NBV figure will sit directly beside a fabricated illustrative
   depreciation figure on the same grid. **(b) NBV for an Asset Category with no
   configured useful life confirmed** (Resolved Question 51) — `computeAssetNbv` returns
   the asset's `purchaseCost` unchanged (not yet depreciated); this confirms
   already-implemented, test-pinned behavior (R-36 in `OPEN-FINDINGS.md`), not a new
   decision requiring a code change; reachable by data alone because `AssetCategory` is an
   open `string` type, not an enum of the five categories.
2. **[§8 P-002 Main Dashboard](#8-p-002-main-dashboard)** and **[§20 P-014 Executive
   ...
   one another) are both rewritten: the KPI grid section heading, ASCII diagram, and
   tile-enumeration paragraph change from a nine-tile "as built" description to a
   **specified ten-tile** description (nine built and live, NBV specified but not built),
   matching Design v0.17 §13's "Logical Dashboard — Current MVP (As Built) + Specified
   Tenth Tile" framing. The "NBV/Risk/Utilization — Status" subsection's NBV bullet is
   rewritten: every prior statement that "no NBV tile exists on this dashboard today, and
   none can be built" (an absence statement) is corrected to a **presence** specification
   — the tile is now specified (placement + formula confirmed) — while preserving the
   fact that it is still **NOT built and NOT yet buildable**, now because the five default
   useful-life values are missing (Open Question 3a), not because the tile itself was
   previously undefined. No placeholder or illustrative NBV *value* is shown anywhere.
3. **[§23A P-018 Settings](#23a-p-018-settings)** updated: a new "NBV for an Asset Category
   with No Configured Useful Life" subsection is added under the existing NBV section,
   recording the Resolved Question 51 behavior and its rejected alternatives (0, NaN,
   exclusion). The section's own status (**shape confirmed only, NOT built**) is
   unchanged — this addition does not claim any new code exists. The Traceability line is
   updated to also cite Resolved Questions 50–51 / Design §13.
4. **[§27 Prototype Traceability Matrix](#27-prototype-traceability-matrix)**'s P-014 and
   P-018 rows are updated to record both confirmations. No row was added or removed.
5. **Every existing traceability reference back to `RAISE-FR-EXEC-001` is kept intact.**
   PRD §16 Open Question 3a (the five per-Asset-Category default useful-life values)
   **remains fully OPEN** — no illustrative or placeholder number is invented, suggested,
   or used as an example anywhere in this pass, including in any wireframe. Open Finding
   **F-03 stays OPEN**, narrowed by this update, not closed. No other screen, requirement
   ID, or row is touched by this pass.
6. **`RAISE-PRD.md` and `RAISE-DESIGN.md` are not modified by this pass.** No
   `## NEEDS_PRD_CONFIRMATION` signal is raised — both decisions are already
   business-confirmed, dated 2026-09-07.
7. Header metadata updated: Version bumped to 0.18; PRD Source updated to v0.19 (advanced
   from v0.18); Design Source updated to v0.17 (advanced from v0.16), with the updated §13
   framing added to the cited section list.

**Change Log — v0.16 → v0.17 (2026-09-05, PRD §16 Resolved Question 49 / Design §14
"Header Bell — Second Surface Over the Same Derivation," per explicit business
confirmation):**

1. **Root confirmation.** PRD v0.18 §16 Resolved Question 49 and Design v0.16 §14
   jointly confirm the header bell in RAISE's own `frontend/src/components/AppShell.tsx`
   is a **different artifact** from the ESAPS reference page
   `esaps_ai_template/src/pages/NotificationCenter.tsx` (which PRD §16 Resolved Question
   35 already, and still, keeps entirely out of RAISE scope). The bell is confirmed **in
   scope** for `RAISE-FR-ALERT-001`, as a second display surface over the exact same
   read-time alert derivation **[§18 P-012 Alerts](#18-p-012-alerts)** already uses (both
   consume the shared hook `frontend/src/hooks/useAlerts.ts`) — not a second Alert model,
   not a new requirement, not a new screen ID.
2. **[§6 Global Layout](#6-global-layout)** — new subsection "Header Bell (`AppShell`) —
   Second Surface of P-012 Alerts, AS BUILT" added, describing the shipped behavior:
   badge shows the total alert count (live-verified reading 19), dropdown lists exactly
   five rows (first five in P-012's own severity ordering, live-verified identical to
   P-012's first five rows: `AST-0003 · iPhone 15 Pro` through `AST-0007 · Cisco Catalyst
   9300`, all High), a "View all alerts" link to `/notifications`, and now-present
   `aria-label`/`aria-expanded` (previously no accessible name at all). Explicitly no
   acknowledge/dismiss/read-unread/snooze affordance — badge counts alerts that
   currently exist, not an unseen count.
3. **[§18 P-012 Alerts](#18-p-012-alerts)** updated: new Status Note added recording the
   resolution; a new "Ordering Rationale" subsection records, once, why both surfaces use
   "first five in severity order" rather than "5 most recent" (`Alert` has no timestamp
   of any kind; sorting by the underlying records' own dates was considered and
   rejected); the former "Header bell-icon dropdown — still unreconciled" Open Question
   bullet is struck through and marked Resolved with a cross-reference; the Traceability
   line now names the bell as a second surface over the same requirement.
4. **§5 Screen Inventory and §27 Prototype Traceability Matrix** — P-012 rows updated to
   record the resolution and cross-reference §6 Global Layout. No row was added or
   removed; the bell is recorded as part of P-012's existing row, not a new screen ID.
5. **Every existing traceability reference back to `RAISE-FR-ALERT-001` is kept intact.**
   PRD §16 Resolved Question 35 (ESAPS page out of scope) is unaffected and unchanged;
   PRD §16 Open Question 22a (per-user alert filtering) is unaffected and stays open. No
   other screen, requirement ID, or row is touched by this pass.
6. **`RAISE-PRD.md` and `RAISE-DESIGN.md` are not modified by this pass.** No
   `## NEEDS_PRD_CONFIRMATION` signal is raised — this decision is already
   business-confirmed, dated 2026-09-05.
7. Header metadata updated: Version bumped to 0.17; PRD Source updated to v0.18
   (advanced from v0.17); Design Source updated to v0.16 (advanced from v0.15), with the
   updated §14 framing added to the cited section list.
