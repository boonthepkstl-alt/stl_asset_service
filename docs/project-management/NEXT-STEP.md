...
- **Current status:** 🟢 All checks green — `tsc --noEmit`, `npm run lint` (0 warnings), `npx vitest run` (142/142), `npm run build` all pass; browser-verified live (`/categories` shows all 5 categories with correct counts, expands to real assets, click-through to Asset Detail works, sidebar nav entry reaches the screen).
