...
| **Status** | 🟡 Plain-count KPI backend shipped · NBV/Risk formulas still not started (blocked) · F-22 spec corrected 2026-08-31, re-execution pending |
...
| **Risks** | Building real NBV/Risk formulas here before Phase 6's Oracle integration lands still risks a KPI that has to be recalculated once real financial data is wired in — unchanged risk, not yet triggered since this cut didn't touch those tiles. Formal test execution (`TC-EXEC-001`, 2026-08-26, and `TC-DASH-01..03`, 2026-08-29) confirmed the built page's tile/section names didn't match Prototype P-002/P-014 at all — tracked as **F-22**. **Resolved via business decision 2026-08-31** ("แก้ Prototype ให้ตรงกับของจริง"): the full chain (Design §13 v0.9, Prototype P-002/P-014 v0.8, AC v0.7, Test Plan v0.7, Test Cases v0.7, Traceability Matrix v0.7) now documents the actually shipped dashboard, with NBV/Risk/Utilization retained as a distinct, not-yet-scheduled enhancement, not deleted. This was a documentation correction, not a fresh test execution — a formal re-execution sweep against the corrected spec has not yet run and is the recommended next step to actually close F-22. |
| **Checkpoint** | [PR #33](https://github.com/boonthepkstl-alt/stl_asset_service/pull/33); `CHECKPOINT-2026-08-29-004` (`TS-DASH` sweep, broadens F-22); `CHECKPOINT-2026-08-31-002` (F-22 chain-wide spec correction) — see `PROJECT-CHECKPOINTS.md`. |
...
| **Actual Start** | — |
| **Actual Completion** | — |
| **Status** | ⚪ Not started — fully blocked. Formal test execution confirms Natural Language Search specifically has no matching implementation at all, not just an unresolved PRD question |
...
| **Risks** | Duplicate Detection's matching threshold and merge-vs-flag workflow were explicitly asked of the business on 2026-08-21 and left unanswered (PRD §16 Open Question 20a) — this is the most concretely stalled item in the entire backlog, not merely unprioritized. Formal test execution (`TC-AI-SEARCH-001-01..03` and `TC-AI-STATES-01..05`, 2026-08-29) confirms a second, independent gap: two "AI" surfaces exist in the built app (a no-input header drawer, and a hardcoded keyword-filter box on the Assets page ported from legacy ESAPS), neither answering a natural-language question or exhibiting any of the 5 required response states per Prototype P-015. Tracked as **F-33**, distinct from F-06's citation-format question. |
| **Checkpoint** | `CHECKPOINT-2026-08-29-007`/`-008` (`TS-AI-SEARCH-001`/`TS-AI-STATES` sweeps, docs-only, F-33) — see `PROJECT-CHECKPOINTS.md`. Otherwise none yet — not started. |
