...
| `NotificationCenter.tsx` | *None* | **DO NOT USE** | **Corrected 2026-09-05 — this row was wrong.** It previously read `RAISE-FR-ALERT-001` / **EXTEND** / "Matches MVP's single-channel in-app alerts requirement", which contradicted [`RAISE-PRD.md`](../01-requirements/RAISE-PRD.md) §16 Resolved Question 35 (2026-08-21) — decided the *same day* this document was written — where this page is one of seven ESAPS pages confirmed **entirely out of RAISE scope**, receiving no Traceability ID at any tier. The contradiction was tracked as **Gap 17** in `RAISE-TRACEABILITY-MATRIX.md` and resolved 2026-09-05 in favour of the PRD (§16 **Resolved Question 49**), consistent with this document's own source table, which names the PRD chain the **Sole requirement authority**. **Do not read this as rejecting in-app alerts:** `RAISE-FR-ALERT-001` is P0/MVP and is built — the Alerts screen (P-012) at `/notifications`, plus the header bell in RAISE's own `AppShell.tsx`, which is a **different artifact** that was never ported from here and is in scope per Resolved Question 49. What is out of scope is *this ESAPS page*. |
