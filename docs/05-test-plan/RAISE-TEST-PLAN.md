...
**Source:** [`RAISE-ACCEPTANCE-CRITERIA.md`](../04-acceptance-criteria/RAISE-ACCEPTANCE-CRITERIA.md) v0.16, cross-checked against [`RAISE-PRD.md`](../01-requirements/RAISE-PRD.md) v0.18, [`RAISE-DESIGN.md`](../02-design/RAISE-DESIGN.md) v0.15, [`RAISE-PROTOTYPE.md`](../03-prototype/RAISE-PROTOTYPE.md) v0.17
...
| TS-DASH | AC-DASH | P-002 | L1 | P0 | Partial — AC-DASH-01, -02, and -03a are testable now against the as-built **nine-tile** KPI grid / 10-section list (AC rewritten 2026-08-31 to match `frontend/src/pages/Dashboard/index.tsx`, Open Finding F-22; grid updated 2026-09-05, PRD §16 Resolved Questions 46–48, to add the built, live **Utilization** tile, PR #102). AC-DASH-03a (Utilization) is a **passing, testable** criterion — percentage value plus an "N of M assignable" sub-label, per the confirmed definition/mechanics (PRD §16 Resolved Questions 27/29). AC-DASH-03b (NBV) remains **NOT TESTABLE YET** — the formula is confirmed (PRD §16 Resolved Question 46), the sole remaining blocker is the missing default per-Asset-Category useful-life values (PRD §16 Open Question 3a; Open Finding F-03, OPEN, narrowed not closed) — this is "specified but not yet buildable," not "unspecified." AC-DASH-03c (Risk) is **confirmed out of MVP scope by business decision** (PRD §16 Resolved Question 47) — not a blocked item, not NOT TESTABLE YET, not counted as a gap |
...
| TS-DASH | Q3a NBV per-Asset-Category default useful-life values, tracked as [Open Finding F-03](../project-management/OPEN-FINDINGS.md#blocking-gates-an-mvp-requirement) (OPEN, narrowed 2026-09-05, not closed) | **Updated 2026-09-05 (PRD v0.17 §16 Resolved Questions 46–48, per confirmed business decision):** the KPI grid grew from eight to **nine** tiles with the addition of a built, live **Utilization** tile (PR #102, 2026-09-05). AC-DASH-01 and AC-DASH-02 test the actual shipped nine-tile KPI grid (Total Assets, Available, Assigned, **Utilization**, In Maintenance, Expired Warranty, Software Licenses, Monthly Depreciation, Monthly Cost) and 10-section list — both fully testable, no remaining gap on tile/section presence. The former single AC-DASH-03 ("none of NBV/Risk/Utilization is present") is **restructured into three separately-reasoned sub-criteria**, because the three KPIs are no longer in the same state: **AC-DASH-03a (Utilization)** is now a **passing, testable** criterion — a percentage value plus an "N of M assignable" sub-label, per the confirmed definition/mechanics (PRD §16 Resolved Questions 27/29) — not a blocked item. **AC-DASH-03b (NBV)** remains **NOT TESTABLE YET**: the formula itself is confirmed (PRD §16 Resolved Question 46 — straight-line depreciation, salvage value zero, clamped at 0, useful life per Asset Category via P-018); the sole remaining blocker is the missing default per-Asset-Category useful-life values (PRD §16 Open Question 3a) — a "specified but not yet buildable" state, distinct from an "unspecified" one, since the next action is "supply the default values," not "define the formula." **AC-DASH-03c (Risk)** is **confirmed out of MVP scope by business decision** (PRD §16 Resolved Question 47) — the Executive/Main Dashboard will not carry a Risk KPI tile for MVP; this is explicitly **not** a blocked item, **not** NOT TESTABLE YET, and **not** counted as a gap, and is excluded from this Blocked Items list on that basis (see AC document §20, which removes Risk from its Not-Yet-Testable Summary table entirely rather than narrowing it) |
...
| TS-DASH | AC-DASH | Product / Dashboard | As-built `frontend/src/pages/Dashboard/index.tsx` (Prototype §8, nine-tile grid including built, live Utilization, PR #102); §13 Executive Intelligence / §5.4 P-018 NBV Settings section cover only the not-yet-built NBV proposal KPI (AC-DASH-03b); Risk (AC-DASH-03c) is confirmed out of MVP scope, no design area applies | P-002 |
...
**Version:** 0.16 (re-synced against `RAISE-ACCEPTANCE-CRITERIA.md` v0.16, 2026-09-07 —
AC-ALERT-001-12 corrected in place: the closed header bell button shows only a dot
presence indicator, not a numeral; the numeral is exposed only via the accessible name
and the dropdown panel header badge. This is an AI-introduced specification-drafting
correction, not a business decision and not a product defect — the product itself is
unchanged. This suite's blocked-item count is unaffected)
