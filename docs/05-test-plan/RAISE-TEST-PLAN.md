...
**Source:** [`RAISE-ACCEPTANCE-CRITERIA.md`](../04-acceptance-criteria/RAISE-ACCEPTANCE-CRITERIA.md) v0.17, cross-checked against [`RAISE-PRD.md`](../01-requirements/RAISE-PRD.md) v0.19, [`RAISE-DESIGN.md`](../02-design/RAISE-DESIGN.md) v0.17, [`RAISE-PROTOTYPE.md`](../03-prototype/RAISE-PROTOTYPE.md) v0.18
...
on the missing default per-Asset-Category useful-life values (PRD §16 Open
Question 3a; see §8 below). This is the same criterion-level-not-suite-level
block pattern already applied to TS-DASH and TS-EXEC-001 (§8) — the Warranty
section's own criteria are unaffected and execute normally.
...
| TS-EXEC-001 | AC-EXEC-001 | RAISE-FR-EXEC-001 | As-built `frontend/src/pages/Dashboard/index.tsx` (Prototype §20, same built page as P-002/TS-DASH, **ten-tile** grid as of 2026-09-07 — PRD §16 Resolved Question 50 — including built, live Utilization, PR #102, and the not-yet-built NBV tenth tile); §13 Executive Intelligence / §5.4 P-018 NBV Settings section cover the not-yet-built NBV tile (AC-EXEC-001-03b, presence criterion, and AC-EXEC-001-04, unconfigured-Asset-Category rule, PRD §16 Resolved Question 51) — both NOT TESTABLE YET, blocked on the tile not being built and on PRD §16 Open Question 3a; Risk (AC-EXEC-001-03c) is confirmed out of MVP scope, no design area applies | P-014 |
...
**Version:** 0.17 (re-synced against `RAISE-ACCEPTANCE-CRITERIA.md` v0.17, 2026-09-07 —
propagates PRD §16 Resolved Questions 50 and 51: the KPI grid on P-002/P-014 is
re-specified from nine tiles to ten tiles, adding NBV as a tenth tile, and a new
unconfigured-Asset-Category NBV rule is recorded. Both remain NOT TESTABLE YET, blocked
on the tile not being built and on PRD §16 Open Question 3a, which remains fully OPEN —
no useful-life number is invented anywhere in this document)

**Change Log — v0.16 → v0.17 (2026-09-07, propagating PRD §16 Resolved Questions 50–51 via
AC document v0.17, closing Open Finding F-52's specification gap; no code written, no
useful-life value supplied):**
