...
/* Lightweight SVG charts — no external deps. Ported verbatim from src/components/Charts.tsx
   (pure presentational, no data fetching, so no behavior changes needed for the migration). */

// DonutChart callers pass a `bg-*` class per segment (for the legend swatch). The SVG stroke
// needs the matching `text-*` class so `stroke="currentColor"` picks it up — but Tailwind's
// scanner only emits a utility for a class string that appears literally in source, so deriving
// it at runtime via `.replace('bg-', 'text-')` silently produces an ungenerated class (segment
// renders in the inherited/black text color instead of its intended hue). This table spells out
// the `text-*` counterpart literally so Tailwind always generates it; extend it when a new `bg-*`
// color is introduced at a call site.
const DONUT_STROKE_CLASS: Record<string, string> = {
  ...
  'bg-surface-500': 'text-surface-500',
...
export function BarChart({ data, height = 200, color = 'bg-brand-500', color2 = 'bg-accent-400' }: BarChartProps) {
  ...
  return (
    <div className="flex items-end gap-3 h-full" style={{ height }}>
      {data.map((d) => (
        <div key={d.label} className="flex-1 flex flex-col items-center gap-1.5 group h-full">
          ...
          {data.map((d, i) => {
            ...
            const el = (
              <circle
                ...
                className={DONUT_STROKE_CLASS[d.color] ?? d.color.replace('bg-', 'text-')}
      ...
      {data.map((d) => (
        <div key={d.label}>
          ...
          <div className="h-2 w-full rounded-full bg-surface-200 overflow-hidden">
            <div className={cn('h-full rounded-full transition-all duration-500', d.color)} style={{ width: `${d.max > 0 ? (d.value / d.max) * 100 : 0}%` }} />
          </div>
        </div>
      ))}
    </div>
