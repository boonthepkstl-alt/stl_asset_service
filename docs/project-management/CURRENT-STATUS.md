...
**As of:** 2026-08-27, after `CHECKPOINT-2026-08-27-001` (F-24 fix — not
...
Financial/Lifecycle sections) is closed (R-07) — **`RAISE-FR-ASSET-001`
now `PASS` on all six test cases**, up from `FAIL (partial)`.
...
Custody History (found F-23 through F-26). Of those, **F-25 and F-26
remain open** — F-25 (no Category & Hierarchy screen) and F-26 (Custody
History not append-only) still gate `RAISE-FR-ASSET-002` (`FAIL (partial)`)
and `RAISE-FR-ASSET-003` (`FAIL`) respectively. Every development session
should close out per
...
**Buildable now:** **F-26** (Custody History not append-only) — F-23 and
F-24 are both now fixed (R-06, R-07). F-26 is non-PRD-blocked but touches
state-mutation logic (Check-in), a slightly higher-risk change than the
two prior display-only fixes. F-25 (no Category & Hierarchy screen)
remains the largest of the two remaining findings (a whole new screen)
and closest to "needs a scoped-down first cut."
