...
(#16-open-questions), closing [§16 Open Question 3a](#16-open-questions)): business
answered DR-02 verbatim in Thai — "ใช้ 5 ปีทุกอย่างยกเว้นมือถือ 3 ปี" ("use 5 years for
everything except mobile phones, 3 years"). That phrasing carried two ambiguities, both
put back to business and answered explicitly rather than inferred: (1) the Mobile
Category contains two Asset Types (Smartphone, Tablet) — asked whether "มือถือ" meant
the whole category or only the handset, business answered **Tablet = 5 years**;
Smartphone is the only Type at 3 years. (2) asked whether "everything = 5" is a blanket
default for future Asset Types (`Asset.type` is free text), business answered **no — it
applies only to the 10 Asset Types present in the data today**, not as a default for
unlisted types. **Confirmed values:** Laptop 5, Monitor 5, Headphones 5, **Smartphone
3**, Tablet 5, Printer 5, Projector 5, Server 5, Router 5, Camera 5 (years). **No value
beyond these ten is supplied, invented, or implied.** [§16 Resolved Question 51]
(#16-open-questions) (unconfigured lookup key returns `purchaseCost` unchanged) remains
**unaffected and unchanged** and is precisely the rule governing any Asset Type outside
this table — which is why business declined a blanket default. **Not touched by this
decision:** the NBV formula ([§16 Resolved Question 46](#16-open-questions), straight-line,
zero salvage, clamped at 0) and the per-Asset-Type keying ([§16 Resolved Question 52]
(#16-open-questions)) are both already confirmed and untouched — this decision supplies
only the numeric values for that existing key; NBV tile placement ([§16 Resolved
Question 50](#16-open-questions)) is also untouched. `RAISE-FR-EXEC-001`'s Priority
(P0), Scope (MVP), and Status are unchanged; no code change is made by this PRD-only
decision itself. **This PRD update was scoped to `RAISE-PRD.md` only** — Design/
Prototype/Acceptance Criteria/Test Plan/Test Cases/Traceability Matrix need their own
sync pass; `OPEN-FINDINGS.md` (Open Finding F-03 closure) and `DECISION-REQUESTS.md`
(DR-02 closure) are updated separately, in session close-out, not by this edit. Prior
(v0.21): Business decision confirmed 2026-09-08, business confirmation
via direct chat session, about `RAISE-FR-MAINT-001`'s already-shipped per-priority SLA
target hours (new [§16 Resolved Question 53](#16-open-questions)): business confirmed
the four values as they stand — **Critical 2h, High 8h, Medium 24h, Low 48h** — from
three options put to them (confirm as-is; supply different values; decide MVP makes no
SLA promise at all). Business chose **confirm as-is**. **This confirms already-shipped
behaviour rather than requesting a change — no code change follows.** Implemented at
`frontend/src/services/ticket-service.ts:14` (`SLA_HOURS` map) and
`go-template-main/service/ticketService.go:19-26` (the same map, whose own comment says
it mirrors the frontend map exactly); every created ticket is stamped with
`slaTargetHours` from that map, and the priority selector labels each choice with its
SLA (e.g. "Critical (2h SLA)"). **Origin recorded honestly:** these values were
**inherited from the ESAPS reference app** (`src/data/requisitionData.ts:231,328` seed
`slaTargetHours: 8` and `24`), which `CLAUDE.md` classes as REFERENCE ONLY — inheritance
from it was never authority, which is why this had never been confirmed by business
until now. **Raised as Open Finding F-54** (2026-09-08) in `OPEN-FINDINGS.md`
(maintained separately, not edited by this change) because the product was shipping and
displaying SLA commitments while the PRD listed SLA as TBD. **This decision resolves
only the single overall per-priority SLA target per ticket — it does NOT resolve "SLA
per stage".** [§8 `RAISE-FR-MAINT-001`'s Open Question row](#8-functional-requirements)
and [§16 Q14](#16-open-questions) both still list SLA **per stage** (how long each of
the four workflow stages — User Requisition → Dept Approval (Delegated) → IT Dispatch →
Technician Execution — may take) as **TBD**; no per-stage budget is supplied or
implemented by this decision. The vendor model, cost model, and delegated-approver
configuration rules also remain exactly as open as before — business said nothing about
them. `RAISE-FR-MAINT-001`'s Priority (P0), Scope (MVP), and Status are **unchanged**;
its Compliance Review verdict (`PASS`, resting on the confirmed 4-stage transitions from
[§16 Resolved Question 33](#16-open-questions), with SLA deliberately excluded as NOT
TESTABLE YET so no executed test asserts an SLA value) is **untouched**. **No numeric
value beyond the four confirmed above is written anywhere by this decision.** Prior
(v0.20): Business decision confirmed 2026-09-08, business confirmation
via direct chat session, about `RAISE-FR-EXEC-001`'s NBV useful-life configuration key
(new [§16 Resolved Question 52](#16-open-questions)): the NBV useful life is configured
**per Asset Type (the sub-category)**, not per Asset Category — this **amends, without
deleting,** [§16 Resolved Question 46](#16-open-questions)'s 2026-09-05 statement that
useful life is "configurable per Asset Category." Business, asked for the five
per-Category values, stated IT Hardware has no fixed figure ("ไม่มีกำหนดตายตัว แล้วแต่อุปกรณ์
ที่ซื้อ" — depends on the equipment purchased) while the other four categories can each
carry one; of four options put to business (per Type; per Asset with a new field; a
Category default plus per-asset override; or naming one representative per-Category
value), business chose **per Type**, because a per-Type table is a strict superset of a
per-Category one — the four fixed categories simply repeat the same value across their
types. `Asset.type` already exists end-to-end
(`go-template-main/sql/pg/V1__Assets_Table.sql:7`, `go-template-main/model/assetModel.go:23`,
and the frontend `Asset` type/fixtures) — same precedent as the F-27 resolution that
`type` **is** the sub-category of `category`. No new field or data model is needed.
**No numeric useful-life value for any category or type is supplied, invented, or
illustrated by this decision** — [§16 Open Question 3a](#16-open-questions) remains
fully **OPEN**, now re-scoped to ask for one value **per Asset Type** rather than per
Asset Category. [§16 Resolved Question 51](#16-open-questions) (NBV for an
unconfigured lookup key returns `purchaseCost` unchanged) is **unaffected and
unchanged** — `type` is free-text exactly as `category` is, so a newly-added type with
no configured value already falls under that existing rule; no code change follows
from either question. `RAISE-FR-EXEC-001`'s Priority/Scope/Status and Compliance
Review verdict are unchanged. **Known downstream consequence, not acted on here:** the
same-day (2026-09-08) chain sync for [§16 Resolved Questions 50–51](#16-open-questions)
(Design v0.17, Prototype v0.18, Acceptance Criteria v0.17, Test Plan v0.17, Test Cases
v0.27, Traceability Matrix v2.7) specified the configuration shape as
`NBVSettings: Record<AssetCategory, usefulLifeYears>`, keyed by Category — this
Resolved Question 52 supersedes that key, so those six documents need a **second sync
pass**; none of them is edited by this change. Prior (v0.19): Two business decisions
confirmed 2026-09-07, business confirmation via direct chat session, about
`RAISE-FR-EXEC-001`'s NBV KPI (new [§16 Resolved Questions 50–51](#16-open-questions)):
(1) the NBV KPI tile is added as a
...
| NBV KPI Definition | **Resolved 2026-09-05** (business confirmation, direct chat session — see [§16 Resolved Question 46](#16-open-questions)): **straight-line depreciation** — NBV = purchaseCost − (purchaseCost ÷ usefulLifeYears × assetAgeInYears), computed from the existing `purchaseDate` and `purchaseCost` fields already present end-to-end on every Asset record (`frontend/src/types/asset.ts`, `go-template-main/model/assetModel.go`, `sql/pg/V1__Assets_Table.sql`) — no new Asset field or data model is needed. **Useful life configuration key AMENDED 2026-09-05→2026-09-08:** originally recorded as configurable per Asset Category (Resolved Question 46); **business confirmed 2026-09-08 (see [§16 Resolved Question 52](#16-open-questions)) that the key is per Asset Type (the sub-category, `Asset.type`) instead** — a per-Type table is a superset of a per-Category one, so this does not reopen the method (straight-line, zero salvage, clamped at 0 are unchanged), only the configuration key, still set by an admin in Settings, same precedent as [§16 Resolved Question 41](#16-open-questions)'s per-category Warranty "Expiring" threshold. Salvage/residual value is **zero**; NBV is **clamped at 0** so it never goes negative. **Default useful-life values per Asset Type CONFIRMED 2026-09-23** (business confirmation, direct chat session, resolving Decision Request DR-02 — see [§16 Resolved Question 54](#16-open-questions), closing [§16 Open Question 3a](#16-open-questions)): Laptop 5, Monitor 5, Headphones 5, **Smartphone 3**, Tablet 5, Printer 5, Projector 5, Server 5, Router 5, Camera 5 (years) — these are the only 10 Asset Types present in the data today; the figure is **not** a default for any future/unlisted Asset Type, which instead falls under [§16 Resolved Question 51](#16-open-questions) (returns `purchaseCost` unchanged). Do not invent or use a value beyond these ten as if confirmed, for any category or any type. **Supporting fact recorded to prevent a wrong assumption:** the Asset record's existing `currentValue` field **cannot** serve as NBV — `go-template-main/service/assetService.go:101` sets `currentValue` equal to `purchaseCost` on create and never recomputes it, and seed fixtures hold hand-written values at inconsistent rates (asset `a1` is 85% of cost after 1.6 years; `a5` is 60% after 3.8 years). |
...
| Open Question | **NBV KPI fully resolved as of 2026-09-23** (Open Finding F-03 now closed — its closure is recorded in `OPEN-FINDINGS.md` separately): NBV formula/method is confirmed (see NBV KPI Definition row), NBV KPI tile placement is confirmed ([§16 Resolved Question 50](#16-open-questions)), NBV behavior for an unconfigured lookup key is confirmed ([§16 Resolved Question 51](#16-open-questions)), the configuration key is confirmed as **per Asset Type** ([§16 Resolved Question 52](#16-open-questions)), and the **ten default useful-life values per Asset Type are now confirmed** (see NBV KPI Definition row and [§16 Resolved Question 54](#16-open-questions), closing [§16 Open Question 3a](#16-open-questions)). No numeric value beyond those ten has been supplied, invented, or illustrated. Risk KPI is confirmed NOT MVP (see Risk KPI (MVP Status) row) — [§16 Q4](#16-open-questions) remains open but no longer blocks this requirement. Utilization is fully resolved and built (see [§16 Resolved Questions 27, 29, 48](#16-open-questions)). Dashboard layout itself remains undefined — see [§16 Q3](#16-open-questions). |
...
3. What is the exact definition of utilization (Executive Dashboard KPI)? — **Resolved
   ...
   (straight-line depreciation, zero salvage, clamped at 0, per-Asset-Type
   configurable useful life per [Resolved Question 52](#16-open-questions)) — the
   *default useful-life values per Asset Type* are now **confirmed, 2026-09-23, see
   [Resolved Question 54](#16-open-questions)**, closing [Open Question 3a]
   (#16-open-questions) below. **Risk KPI confirmed NOT
...
3a. **CLOSED 2026-09-23, see [Resolved Question 54](#16-open-questions).** Business
    answered Decision Request DR-02 with the per-Asset-Type useful-life values —
    5 years for all 10 current Asset Types except **Smartphone (3 years)**; **Tablet is
    confirmed at 5 years** (a distinct Asset Type from Smartphone within the Mobile
    Category); the 5-year figure is confirmed to apply **only** to the 10 Asset Types
    present in the data today, **not** as a blanket default for any future/unlisted
    Asset Type — [Resolved Question 51](#16-open-questions) (unconfigured key returns
    `purchaseCost` unchanged) governs those instead. See Resolved Question 54 for the
    full values table and both clarifying rounds. **This question is now CLOSED — no
    numeric value beyond the ten confirmed there is supplied, invented, or implied.**
    History below retained for traceability of how the question was originally framed
    and re-scoped. **[Re-scoped 2026-09-08, see Resolved Question 52]** What are the default
    useful-life values **per Asset Type** (the sub-category, e.g. Laptop, Monitor,
    Smartphone — one value per `type` value present in the data, a free-text field
    that grows as assets are added, not a fixed enumeration) for NBV straight-line
    depreciation (`RAISE-FR-EXEC-001`)? — Originally raised alongside
    [Resolved Question 46](#16-open-questions), 2026-09-05, as five values **per
    Asset Category** (IT Hardware, Mobile, Office Equipment, Infrastructure, Media
    Equipment). Business was asked directly and answered **"I will specify these
    myself"** — the numeric values have not been supplied. This is the same shape as
    [Resolved Question 41](#16-open-questions)/[Open Question 15b](#16-open-questions):
    the mechanism (per-Type configurability, confirmed 2026-09-08) is confirmed, the
    exact numbers are not. **Do not invent or use an illustrative number as if
    confirmed, for any category or any type.** Tracked as the narrowed remainder of
    Open Finding **F-03** in `OPEN-FINDINGS.md` (maintained separately, not edited by
    this change). **Further narrowed 2026-09-07:** the two remaining adjacent NBV
    questions that were also outstanding — where the NBV tile is placed on the
    dashboard KPI grid, and what NBV returns for an Asset whose lookup key has no
    configured useful life — are now both settled, see
    [Resolved Question 50](#16-open-questions) and [Resolved Question 51]
    (#16-open-questions) (RQ51 is keyed the same way regardless of Category vs. Type
    and is unaffected by the RQ52 re-key). **Re-scoped 2026-09-08, see
    [Resolved Question 52](#16-open-questions):** the configuration key is now
    confirmed as **per Asset Type**, not per Asset Category — this question therefore
    now asks for **one numeric value per Asset Type present in the data**, not five
...
14. What maintenance information is required? — **Partially resolved 2026-08-21, see
    ...
    confirmed. **Further partially resolved 2026-09-08, see [Resolved Question 53]
    (#16-open-questions):** the already-shipped **per-priority SLA target hours**
    (Critical 2h, High 8h, Medium 24h, Low 48h) — a single overall target per ticket,
    keyed by priority — are confirmed as they stand. **This does NOT resolve SLA per
    stage**, which is a different question and **remains open**: no budget has been
    supplied or implemented for how long any of the four individual workflow stages may
    take. **SLA per stage, vendor model, cost model, and delegated-approver
...
46. **What is the NBV (Net Book Value) formula for the Executive Dashboard KPI
    ...
    **AMENDED 2026-09-08, see [Resolved Question 52](#16-open-questions) — the
    configuration key stated above (per Asset Category) is superseded, not
    deleted.** This entry's content above is preserved as the historical record of
    what was confirmed 2026-09-05. Business subsequently confirmed the useful-life
    configuration is keyed by **Asset Type (the sub-category)**, not Asset Category —
    a per-Type table is a superset of a per-Category one, so this amendment narrows
    the *mechanism*, not the *method* (straight-line depreciation, zero salvage,
    clamped-at-0 all still stand unchanged). See Resolved Question 52 for the full
    reasoning and evidence. [Open Question 3a](#16-open-questions) is re-scoped
    accordingly and remains OPEN.
...
52. **Is the NBV useful life configured per Asset Category or per Asset Type
    (`RAISE-FR-EXEC-001`)?** — Raised because, asked to supply the five per-Category
    default values confirmed as the mechanism in [Resolved Question 46]
    (#16-open-questions), business could not give a single fixed figure for one of
    the five: IT Hardware. Business confirmed, **2026-09-08, business confirmation,
    direct chat session:**

    **The NBV useful life is configured per Asset Type (the sub-category), not per
    Asset Category.** This **amends [Resolved Question 46](#16-open-questions)**,
    whose recorded mechanism was "configurable per Asset Category" — that entry's
    content is **preserved, not deleted or rewritten**, and is cross-linked to this
    entry as superseded on this one point (see the amendment note appended to
    Resolved Question 46). The straight-line method, zero salvage, and clamp-at-0
    from Resolved Question 46 are **unchanged**.

    **Why business gave this answer:** asked for the five per-Category values,
    business stated IT Hardware has **no fixed figure** — *"ไม่มีกำหนดตายตัว แล้วแต่อุปกรณ์ที่
    ซื้อ"* (no fixed value, it depends on the equipment purchased). Presented with four
    options — (i) per Type using the existing `type` field; (ii) per Asset with a new
    field; (iii) a Category default plus a per-asset override; (iv) keeping
    per-Category and naming one representative value for IT Hardware — business chose
    **(i), per Type**. Asked separately whether only IT Hardware varies or all five
    categories do, business answered **only IT Hardware** — the other four categories
    can each carry one fixed value.

    **How the two answers combine, recorded explicitly because it is the reason for
    the chosen key:** a per-Type table is a **superset** of a per-Category table. For
    the four categories that do have a fixed value, the same value simply repeats
    across that category's types — exactly equivalent to a single category-level
    value. **One uniform per-Type key therefore satisfies both answers** without a
    two-level lookup model and without a special case carved out for IT Hardware.

    **No new field or data model is needed** — `Asset.type` already exists
    end-to-end: `go-template-main/sql/pg/V1__Assets_Table.sql:7` (`type varchar(100)
    NOT NULL`), `go-template-main/model/assetModel.go:23` (`Type string`), and the
    frontend `Asset` type and its fixtures already populate it. This is the **same
    precedent as the F-27 resolution**, which already established that the existing
    `type` field **is** the sub-category of `category`.

    **The types currently present in seeded data** (derived from
    `frontend/src/data/fixtures/mockData.ts`, not invented): IT Hardware → Laptop,
    Monitor, Headphones; Mobile → Smartphone, Tablet; Office Equipment → Printer,
    Projector; Infrastructure → Router, Server; Media Equipment → Camera — ten types
    as seeded today. **This is not recorded as a fixed business rule.** `type` is a
    free-text `varchar`, and the set of values **grows as assets are added**, exactly
    as `category` does. The requirement is **one configured value per type present in
    the data**, not a fixed enumeration of ten.

    **[Resolved Question 51](#16-open-questions) is unaffected and is not changed by
    this decision.** Its rule already covers this case: an Asset whose lookup key has
    **no configured useful life** contributes `purchaseCost` unchanged (treated as
    not yet depreciated) and **stays included** in the portfolio total. Because
    `type` is free-text just as `category` is, a newly-added type with no configured
    value falls under exactly that existing rule. **No code change follows from
    Resolved Question 51 either way.**

    **Still fully OPEN, not touched by this decision:** [Open Question 3a]
    (#16-open-questions), re-scoped by this entry to ask for **one value per Asset
    Type** rather than five values per Asset Category, remains **OPEN**. **No
    numeric useful-life value has been supplied, invented, or illustrated for any
    category or any type, anywhere, by this resolution.** `RAISE-FR-EXEC-001`'s
    Priority (P0), Scope (MVP), and Status are **unchanged**; no Compliance Review
    verdict is touched; no code has been written.

    **Known downstream consequence, flagged not acted on:** the chain was synced
    earlier the same day (2026-09-08) for [Resolved Questions 50–51]
    (#16-open-questions) — Design v0.17, Prototype v0.18, Acceptance Criteria v0.17,
    Test Plan v0.17, Test Cases v0.27, Traceability Matrix v2.7 — and that sync
    specified the configuration shape as
    `NBVSettings: Record<AssetCategory, usefulLifeYears>`, keyed by **Category**.
    **This Resolved Question supersedes that key**, so those six documents now need
    a **second sync pass**; none of them is edited by this change.

    Updated in §8 (`RAISE-FR-EXEC-001` — "NBV KPI Definition" row cross-linked to
    this amendment, "Open Question" row re-scoped), §16 (this entry; Resolved
    Question 46 amendment note; Open Question 3a re-scoped), the
    [§17 Requirement Traceability Matrix](#17-requirement-traceability-matrix) row
    for `RAISE-FR-EXEC-001`, and the [Pre-Finalization Quality Pass]
    (#pre-finalization-quality-pass) (Ambiguous Requirements). **Does not change
    `RAISE-FR-EXEC-001`'s Priority, Scope, Status, or any Compliance Review
    verdict.** This PRD update was scoped to `RAISE-PRD.md` only — no other document
    in the chain was edited by this change.

53. **Are the already-shipped per-priority SLA target hours for `RAISE-FR-MAINT-001`
    confirmed as they stand?** — Raised as **Open Finding F-54** (2026-09-08) in
    `OPEN-FINDINGS.md` (maintained separately, not edited by this change): the product
    was shipping SLA commitments in both tiers and displaying them to users, while the
    PRD listed SLA as TBD, no §16 Resolved Question covered it, Prototype §15 P-009
    listed "Priority (conceptual — TBD)", and Acceptance Criteria §12 marked SLA NOT
    TESTABLE YET. Business confirmed, **2026-09-08, business confirmation, direct chat
    session:**

    **The four per-priority SLA target hours already shipped are confirmed as they
    stand:**
    ...
    Business chose this from three options put to them: (a) confirm the values as
    they stand; (b) supply different values; (c) decide MVP makes no SLA promise at
    all. Business chose **(a)**.

    **This confirms already-shipped behaviour rather than requesting a change — no
    code change follows.** Implemented at
    `frontend/src/services/ticket-service.ts:14` (`const SLA_HOURS:
    Record<CreateTicketInput['priority'], number> = { Critical: 2, High: 8, Medium:
    24, Low: 48 };`) and `go-template-main/service/ticketService.go:19-26` (the same
    map, whose own comment states it "mirrors frontend/src/services/ticket-service.ts's
    SLA_HOURS map exactly"). Every created ticket is stamped with `slaTargetHours` from
    that map, and the priority selector labels each choice with its SLA commitment at
    the moment of filing ("Critical (2h SLA)", "High (8h SLA)", "Medium (24h SLA)",
    "Low (48h SLA)").

    **Origin recorded honestly:** these values were **inherited from the ESAPS
    reference app** (`src/data/requisitionData.ts:231,328`, seeding `slaTargetHours: 8`
    and `24`), not invented by RAISE. `CLAUDE.md` classes that app as REFERENCE ONLY,
    so inheritance from it was never authority on its own — which is exactly why this
    Resolved Question was needed before the values could be treated as a confirmed
    business decision.

    **What this does NOT resolve, stated explicitly because it is the distinction the
    whole entry turns on: "SLA per stage" remains fully OPEN.** [§8
    `RAISE-FR-MAINT-001`](#8-functional-requirements)'s Open Question row and
    [§16 Q14](#16-open-questions)'s remainder both still list "SLA per stage" as TBD,
    and that is a **different question** from what was just answered here:

    - **What was confirmed (this Resolved Question):** a **single overall SLA target
      per ticket, keyed by its priority** — one number per priority level, which is
      what `slaTargetHours` holds.
    - **What remains TBD:** how long **each of the four workflow stages** (User
      Requisition → Dept Approval (Delegated) → IT Dispatch → Technician Execution)
      may take. **No per-stage budget has been supplied, and none is implemented.**

    The **vendor model**, the **cost model**, and the **delegated-approver
    configuration rules** are also left exactly as open as before — business said
    nothing about them; they are not touched by this decision.

    **No number beyond the four confirmed above is written anywhere by this
    decision.** `RAISE-FR-MAINT-001`'s Priority (P0), Scope (MVP), and Status are
    **unchanged**; its Compliance Review verdict (`PASS`, resting on the confirmed
    4-stage transitions from [Resolved Question 33](#16-open-questions)) is
    **untouched** — the Acceptance Criteria deliberately excluded SLA as NOT TESTABLE
    YET, so no executed test asserts an SLA value, and none is introduced by this
    change.

    Updated in §8 (`RAISE-FR-MAINT-001` — Acceptance Criteria, Source Reference, and
    Open Question rows), §16 (this entry; Q14 remainder note updated). **This PRD
    update was scoped to `RAISE-PRD.md` only** — no other document in the chain was
    edited by this change.

54. **What are the default useful-life values per Asset Type for NBV straight-line
    depreciation (`RAISE-FR-EXEC-001`)?** — **Closes [Open Question 3a]
    (#16-open-questions)** and the narrowed remainder of Open Finding **F-03** in
    `OPEN-FINDINGS.md` (maintained separately, not edited by this change — its closure
    is recorded there in session close-out). Business was asked this directly as
    Decision Request **DR-02** (`docs/project-management/DECISION-REQUESTS.md`) and
    answered, **2026-09-23, business confirmation via direct chat session**, verbatim
    in Thai: "ใช้ 5 ปีทุกอย่างยกเว้นมือถือ 3 ปี" ("use 5 years for everything except mobile
    phones, 3 years").

    **That phrasing was ambiguous in two ways. Both were put back to business and
    answered explicitly — neither is inferred by engineering:**

    1. The Mobile Asset **Category** contains two Asset **Types** — Smartphone and
       Tablet. Asked whether "มือถือ" (mobile phone) meant the whole Mobile category or
       only the handset Type, business answered: **Tablet = 5 years.** Smartphone is
       the only Asset Type at 3 years — "มือถือ" meant the handset Type specifically,
       not the Category.
    2. Asked whether "everything = 5 years" is a blanket default that should also apply
       to any future Asset Type not yet present in the data (`Asset.type` is free text,
       confirmed by [Resolved Question 52](#16-open-questions), and new values can
       appear over time), business answered: **no** — the 5-year figure applies **only
       to the 10 Asset Types present in the data today**, not as a default for
       unlisted types.

    **Confirmed values — one entry per Asset Type, all 10 Asset Types currently present
    in the data:**
    ...
    **No value beyond these ten is supplied, invented, or implied by this decision.**
    An Asset Type not in this table (present today outside this list, or added later)
    is explicitly **not** covered by a default — [Resolved Question 51]
    (#16-open-questions) (NBV for an unconfigured lookup key returns `purchaseCost`
    unchanged, treated as not yet depreciated) remains **unaffected and unchanged**,
    and is precisely the rule that governs such a Type; this is why business declined a
    blanket default here rather than it being an oversight.

    **Not touched by this decision:** the NBV **formula** (straight-line, zero salvage,
    clamped at 0 — [Resolved Question 46](#16-open-questions)) and the **per-Asset-Type
    keying** (rather than per-Category — [Resolved Question 52](#16-open-questions))
    are both already confirmed and untouched; this decision supplies only the numeric
    values for that existing key. The NBV KPI tile placement ([Resolved Question 50]
    (#16-open-questions)) and NBV for an unconfigured lookup key ([Resolved Question 51]
    (#16-open-questions)) are likewise untouched.

    **[Open Question 3a](#16-open-questions) is now CLOSED** — see that entry for the
    closure note. `RAISE-FR-EXEC-001`'s Priority (P0), Scope (MVP), and Status are
    unchanged; its Compliance Review verdict is unaffected by this PRD-only change (no
    code change is made by this decision itself — implementing these values in
    Settings/config is downstream engineering work, not yet done as of this entry).

    Updated in §8 (`RAISE-FR-EXEC-001` — NBV KPI Definition and Open Question rows),
    §16 (this entry; [Open Question 3a](#16-open-questions) closed), §17 (Traceability
    Matrix row for `RAISE-FR-EXEC-001`), and the [Pre-Finalization Quality Pass]
    (#pre-finalization-quality-pass) (Ambiguous Requirements). **This PRD update was
    scoped to `RAISE-PRD.md` only** — Design/Prototype/Acceptance Criteria/Test
    Plan/Test Cases/Traceability Matrix are unchanged by this edit and need their own
...
| RAISE-FR-EXEC-001 | Executive Dashboard | MVP | P0 | NBV KPI fully resolved 2026-09-23 (Open Finding F-03 now closed) — Utilization KPI **built and live** (PR #102, merged 2026-09-05, commit 321265f); NBV formula/method confirmed 2026-09-05 (straight-line, zero salvage, clamped at 0); useful-life configuration key **AMENDED 2026-09-08 from per-Asset-Category to per-Asset-Type** (`Asset.type`, same field established as the sub-category by the F-27 resolution) via Settings; the **ten default useful-life values per Asset Type CONFIRMED 2026-09-23** (Laptop/Monitor/Headphones/Tablet/Printer/Projector/Server/Router/Camera = 5 years, Smartphone = 3 years — no default for any other Asset Type), resolving DR-02; NBV KPI tile placement confirmed 2026-09-07 (added as a tenth tile on both P-002/P-014, existing Monthly Depreciation tile unchanged — nine-tile grid must be re-specified/re-executed as ten tiles); NBV for an unconfigured lookup key confirmed 2026-09-07, unaffected by the 2026-09-08 re-key and by the 2026-09-23 value confirmation (keeps existing behavior — returns `purchaseCost` unchanged); Risk KPI confirmed **NOT MVP** for this dashboard (see RAISE-AI-RISK-001, unchanged Pilot/Roadmap status) | v0.1 §8.1; Utilization built 2026-09-05; NBV method and Risk MVP-exclusion confirmed 2026-09-05 (§16 Resolved Questions 46–48); NBV tile placement and unconfigured-category behavior confirmed 2026-09-07 (§16 Resolved Questions 50–51); NBV useful-life configuration key amended to per-Asset-Type 2026-09-08 (§16 Resolved Question 52); NBV per-Asset-Type useful-life numeric values confirmed 2026-09-23, resolving DR-02 (§16 Resolved Question 54) |
...
- **RAISE-FR-EXEC-001 (Executive Dashboard):** ~~"Utilization" is listed as a KPI with no
  ...
  for an Asset with no configured useful life is now confirmed (keeps
  existing behavior — returns `purchaseCost` unchanged, see [§16 Resolved Question 51]
  (#16-open-questions)). **Re-keyed 2026-09-08** — the useful-life configuration key
  itself is **amended from per Asset Category to per Asset Type** (`Asset.type`, the
  sub-category), see [§16 Resolved Question 52](#16-open-questions), which amends
  (without deleting) [§16 Resolved Question 46](#16-open-questions). **Fully resolved
  2026-09-23** — the ten default useful-life values per Asset Type are now confirmed
  (Laptop/Monitor/Headphones/Tablet/Printer/Projector/Server/Router/Camera = 5 years,
  Smartphone = 3 years, no default for any other Asset Type), resolving Decision
  Request DR-02, see [§16 Resolved Question 54](#16-open-questions), which **closes**
  [§16 Open Question 3a](#16-open-questions) and Open Finding **F-03**. This bullet is
  retained (not removed) for traceability of how the ambiguity was originally framed
  and resolved.
...
1. **NBV per-Asset-Type useful-life numeric values CONFIRMED** (new [§16 Resolved
   Question 54](#16-open-questions), closing [§16 Open Question 3a]
   (#16-open-questions) and the narrowed remainder of Open Finding **F-03**): business
   answered DR-02 verbatim in Thai — "ใช้ 5 ปีทุกอย่างยกเว้นมือถือ 3 ปี" ("use 5 years for
   everything except mobile phones, 3 years"). Two ambiguities in that phrasing were
   put back to business and answered explicitly, not inferred: (a) the Mobile Category
   has two Asset Types (Smartphone, Tablet) — business confirmed **Tablet = 5 years**,
   only Smartphone is at 3 years; (b) business confirmed the 5-year figure applies
   **only to the 10 Asset Types present in the data today**, explicitly **not** a
   blanket default for future/unlisted Asset Types. **Confirmed values (years):**
   Laptop 5, Monitor 5, Headphones 5, **Smartphone 3**, Tablet 5, Printer 5,
   Projector 5, Server 5, Router 5, Camera 5. No value beyond these ten is supplied,
   invented, or implied. [§16 Resolved Question 51](#16-open-questions) (unconfigured
   lookup key returns `purchaseCost` unchanged) remains unaffected and governs any
   Asset Type outside this table.
2. **Not touched by this decision:** the NBV formula ([§16 Resolved Question 46]
   (#16-open-questions)) and the per-Asset-Type keying ([§16 Resolved Question 52]
   (#16-open-questions)) are both already confirmed and untouched; this decision
   supplies only the numeric values for that existing key. NBV KPI tile placement
   ([§16 Resolved Question 50](#16-open-questions)) is also untouched.
   `RAISE-FR-EXEC-001`'s Priority (P0), Scope (MVP), and Status are **unchanged**; no
   code change is made by this PRD-only decision itself.
   Updated in §8 (`RAISE-FR-EXEC-001` — NBV KPI Definition and Open Question rows),
   §16 (new Resolved Question 54; [§16 Open Question 3a](#16-open-questions) closed),
   §17 (Traceability Matrix row for `RAISE-FR-EXEC-001`), and the
   [Pre-Finalization Quality Pass](#pre-finalization-quality-pass) (Ambiguous
   Requirements). **This PRD update was scoped to `RAISE-PRD.md` only** —
   Design/Prototype/Acceptance Criteria/Test Plan/Test Cases/Traceability Matrix are
   unchanged by this edit and need their own sync pass; `OPEN-FINDINGS.md` (F-03
   closure) and `DECISION-REQUESTS.md` (DR-02 closure) are updated separately, in
   session close-out, not by this edit.

**Change Log — v0.20 → v0.21 (2026-09-08, business confirmation via direct chat
session, explicit Q&A):**

1. **Already-shipped per-priority SLA target hours for `RAISE-FR-MAINT-001` confirmed
   as they stand** (new [§16 Resolved Question 53](#16-open-questions)): Critical 2h,
   High 8h, Medium 24h, Low 48h — chosen by business from three options put to them
   (confirm as-is; supply different values; decide MVP makes no SLA promise at all).
   **Confirms already-shipped behaviour rather than requesting a change — no code
   change follows.** Implemented at `frontend/src/services/ticket-service.ts:14`
   (`SLA_HOURS` map) and `go-template-main/service/ticketService.go:19-26` (the same
   map). Origin recorded honestly: inherited from the ESAPS reference app
   (`src/data/requisitionData.ts:231,328`, seed `slaTargetHours: 8`/`24`), which
   `CLAUDE.md` classes as REFERENCE ONLY — inheritance was never authority, which is
   why this had not been confirmed by business until now. Raised as Open Finding
   **F-54** (2026-09-08) in `OPEN-FINDINGS.md` (maintained separately, not edited by
   this change).
2. **Does NOT resolve "SLA per stage"**, which remains a separate, still fully **OPEN**
   question: [§8 `RAISE-FR-MAINT-001`'s Open Question row](#8-functional-requirements)
   and [§16 Q14](#16-open-questions)'s remainder both continue to list SLA per stage,
   vendor model, cost model, and delegated-approver configuration rules as TBD. **No
   per-stage budget, and no number beyond the four confirmed above, is supplied,
   invented, or implemented by this change.** `RAISE-FR-MAINT-001`'s Priority (P0),
   Scope (MVP), and Status are **unchanged**; its Compliance Review verdict (`PASS`,
   resting on the confirmed 4-stage transitions from [§16 Resolved Question 33]
   (#16-open-questions)) is **untouched** — the Acceptance Criteria deliberately
   excluded SLA as NOT TESTABLE YET, so no executed test asserts an SLA value.
   Updated in §8 (`RAISE-FR-MAINT-001` — Acceptance Criteria, Source Reference, and
   Open Question rows), §16 (new Resolved Question 53; Q14 remainder note updated).
   **This PRD update was scoped to `RAISE-PRD.md` only** — no other document in the
   chain was edited by this change.

**Change Log — v0.19 → v0.20 (2026-09-08, business confirmation via direct chat
session, explicit Q&A):**

1. **NBV useful-life configuration key amended from per Asset Category to per Asset
   Type** (new [§16 Resolved Question 52](#16-open-questions), amending — not
   deleting — [§16 Resolved Question 46](#16-open-questions)): asked for the five
   per-Category default values, business stated IT Hardware has no fixed figure
   ("ไม่มีกำหนดตายตัว แล้วแต่อุปกรณ์ที่ซื้อ" — depends on the equipment purchased) while the
   other four categories can each carry a fixed value; of four options put to
   business (per Type using the existing field; per Asset with a new field; a
   Category default plus a per-asset override; or naming one representative
   per-Category value), business chose **per Type**, because a per-Type table is a
   superset of a per-Category one — the four fixed categories simply repeat the same
   value across their types. `Asset.type` already exists end-to-end
   (`go-template-main/sql/pg/V1__Assets_Table.sql:7`,
   `go-template-main/model/assetModel.go:23`, and the frontend `Asset` type/fixtures)
   — same precedent as the F-27 resolution that `type` **is** the sub-category of
   `category`. **No new field or data model is needed.** The straight-line method,
   zero salvage, and clamp-at-0 confirmed by Resolved Question 46 are **unchanged**.
2. **Not touched by this decision:** [§16 Open Question 3a](#16-open-questions) —
   re-scoped from five values per Asset Category to one value per Asset Type — is
   **still fully OPEN**; no numeric useful-life value for any category or any type
   is supplied, invented, or illustrated anywhere by this change.
   [§16 Resolved Question 51](#16-open-questions) (NBV for an unconfigured lookup
   key returns `purchaseCost` unchanged) is **unaffected and unchanged** — `type` is
   free-text exactly as `category` is, so this rule already covers a newly-added,
   unconfigured type; no code change follows from Resolved Question 51 either way.
   `RAISE-FR-EXEC-001`'s Priority (P0), Scope (MVP), and Status are **unchanged**;
   its Compliance Review verdict (`PASS (partial)`) is untouched. No code has been
   written.
3. **Known downstream consequence, flagged not acted on:** the same-day chain sync
   for [§16 Resolved Questions 50–51](#16-open-questions) (Design v0.17, Prototype
   v0.18, Acceptance Criteria v0.17, Test Plan v0.17, Test Cases v0.27, Traceability
   Matrix v2.7) specified the configuration shape as
   `NBVSettings: Record<AssetCategory, usefulLifeYears>`, keyed by Category — this
   Resolved Question supersedes that key, so those six documents need a **second
   sync pass**. Updated in §8 (`RAISE-FR-EXEC-001` — "NBV KPI Definition," "NBV for
   Unconfigured Asset Category," and "Open Question" rows), §16 (new Resolved
   Question 52; Resolved Question 46 amendment note; Open Question 3a re-scoped),
   §17 (Traceability Matrix row for `RAISE-FR-EXEC-001`), and the
   [Pre-Finalization Quality Pass](#pre-finalization-quality-pass) (Ambiguous
