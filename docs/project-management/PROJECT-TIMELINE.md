...
| **Risks** | Duplicate Detection's matching threshold and merge-vs-flag workflow were explicitly asked of the business on 2026-08-21 and left unanswered (PRD §16 Open Question 20a) — this is the most concretely stalled item in the entire backlog, not merely unprioritized. Formal test execution (`TC-AI-SEARCH-001-01..03`, 2026-08-29) confirms a second, independent gap: two "AI" surfaces exist in the built app (a no-input header drawer, and a hardcoded keyword-filter box on the Assets page ported from legacy ESAPS), neither answering a natural-language question per Prototype P-015. Tracked as **F-33**, distinct from F-06's citation-format question. |
| **Checkpoint** | `CHECKPOINT-2026-08-29-007` (`TS-AI-SEARCH-001` sweep, docs-only, new finding F-33) — see `PROJECT-CHECKPOINTS.md`. Otherwise none yet — not started. |
